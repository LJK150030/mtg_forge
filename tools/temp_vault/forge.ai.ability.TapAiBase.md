...
fqn: forge.ai.ability.TapAiBase
...
classDiagram
    class TapAiBase {
        -tapTargetList(Player ai, SpellAbility sa, CardCollection tapList, boolean mandatory) boolean
        #tapPrefTargeting(Player ai, Card source, SpellAbility sa, boolean mandatory) boolean
        #tapUnpreferredTargeting(Player ai, SpellAbility sa, boolean mandatory) boolean
        #doTriggerNoCost(Player ai, SpellAbility sa, boolean mandatory) AiAbilityDecision
        +chkDrawback(Player ai, SpellAbility sa) AiAbilityDecision
    }
    TapAiBase --|> SpellAbilityAi : extends
    TapAiBase ..> AiAbilityDecision : uses
    TapAiBase ..> Card : uses
    TapAiBase ..> CardCollection : uses
    TapAiBase ..> Game : uses
    TapAiBase ..> PhaseHandler : uses
    TapAiBase ..> Player : uses
    TapAiBase ..> SpellAbility : uses
```
...
TapAiBase is an abstract base class for AI controllers governing "tap" effects, extending `SpellAbilityAi` to plug into Forge's ability-decision framework. Its responsibility is to decide whether and how the AI should activate a tap ability and, when targeting is required, to select which permanents to tap. It collaborates with `Player`, `Card`, `CardCollection`, and `SpellAbility` to enumerate and filter candidates, consulting `Game` and `PhaseHandler` to time decisions around combat, and returns its verdicts as `AiAbilityDecision` values.

The design splits target selection into a preferred strategy (`tapPrefTargeting`) that favors lethal targets and tapping opponents' potential attackers or blockers based on the current phase, and a fallback (`tapUnpreferredTargeting`) that progressively relaxes criteria when a play is mandatory. The overridden `doTriggerNoCost` and `chkDrawback` hooks drive these strategies for triggered abilities and sub-abilities, while shared helpers like `tapTargetList` centralize the min/max-target and "cast less than max" bookkeeping, leaving concrete subclasses to supply only ability-specific behavior.

## Source
`forge-ai/src/main/java/forge/ai/ability/TapAiBase.java`
