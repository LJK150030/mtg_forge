...
tags:
  ...
  - module/forge-game
  - pkg/forge/game/card
fqn: forge.game.card.CounterEnumType
...
classDiagram
    class CounterEnumType {
        ...
        TICKET
        -String name
        -String counterOnCardDisplayName
        -int red
        -int green
        -int blue
        -CounterAiCategory aiCategory
        +getName() String
        +getRed() int
        +getGreen() int
        +getBlue() int
        +getCounterOnCardDisplayName() String
        +getType(String name) CounterEnumType
        +is(CounterEnumType eType) boolean
        ~CounterEnumType(String counterOnCardDisplayName, CounterAiCategory aiCategory)
        ~CounterEnumType(String counterOnCardDisplayName, int red, int green, int blue, CounterAiCategory aiCategory)
        ~CounterEnumType(String name, String counterOnCardDisplayName, int red, int green, int blue, CounterAiCategory aiCategory)
    }
    CounterEnumType ..|> CounterType : implements
    CounterEnumType ..> CounterAiCategory : uses
```
...
CounterEnumType enumerates every kind of counter that can be placed on a card or player in the Forge engine â€” from standard +1/+1 and loyalty counters to the hundreds of keyword- and mechanic-specific varieties. Each constant carries display metadata: a card-display abbreviation, an RGB tint for rendering, and a CounterAiCategory (Positive, Negative, or Neutral) that lets the AI judge whether gaining a counter is beneficial. As an enum implementing CounterType, it supplies the concrete, fixed set of built-in counter definitions behind that interface, exposing accessors for name, color components, and display name.

The design favors a compact tabular declaration, with overloaded constructors defaulting color to white and deriving a capitalized name from the constant when no explicit name is given. The static getType normalizes user/script strings â€” stripping slashes and mapping "+"/"-" to "p"/"m" â€” so symbolic power/toughness counters resolve by name, while is() provides simple identity comparison.

## Source
`forge-game/src/main/java/forge/game/card/CounterEnumType.java`
...
## Python
`forge/game/card/CounterEnumType.py`
...
class CounterEnumType(CounterType, Enum):
    ...
    def __init__(self, *args):
        if len(args) == 2:
            counterOnCardDisplayName, aiCategory = args
            self._init(counterOnCardDisplayName, 255, 255, 255, aiCategory)
        elif len(args) == 5:
            counterOnCardDisplayName, red, green, blue, aiCategory = args
            self._init(counterOnCardDisplayName, red, green, blue, aiCategory)
        elif len(args) == 6:
            name, counterOnCardDisplayName, red, green, blue, aiCategory = args
            self._init(counterOnCardDisplayName, red, green, blue, aiCategory)
            self._counterName = name

    def _init(self, counterOnCardDisplayName, red, green, blue, aiCategory):
        self._counterName = self.name[0:1].upper() + self.name[1:].lower()
        self.counterOnCardDisplayName = counterOnCardDisplayName
        self.red = red
        self.green = green
        self.blue = blue
    ...
    def getCounterOnCardDisplayName(self) -> str:
        return self.counterOnCardDisplayName

    @staticmethod
    def getType(name: str) -> "CounterEnumType":
        replacedName = name.replace("/", "").replace("+", "p").replace("-", "m").upper()
        return CounterEnumType[replacedName]

    def is_(self, eType: "CounterEnumType") -> bool:
        return self is eType
```
