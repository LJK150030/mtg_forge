...
tags:
  ...
  - module/forge-ai
  - pkg/forge/ai
fqn: forge.ai.AiAttackController
package: forge.ai
module: forge-ai
kind: Class
---
...
```mermaid
classDiagram
    class AiAttackController {
        -List~Card~ attackers
        -List~Card~ blockers
        -List~Card~ oppList
        -List~Card~ myList
        -Player ai
        -Player defendingOpponent
        -int aiAggression
        -boolean nextTurn
        -int timeOut
        -boolean canUseTimeout
        -List~CompletableFuture~ futures
        ~boolean LOG_AI_ATTACKS
        -refreshCombatants(GameEntity defender) void
        +getOpponentCreatures(Player defender) List~Card~
        +removeBlocker(Card blocker) void
        -canAttackWrapper(Card attacker, GameEntity defender) boolean
        +choosePreferredDefenderPlayer(Player ai) Player
        +choosePreferredDefenderPlayer(Player ai, boolean forCombatDmg) Player
        +sortAttackers(List~Card~ in) List~Card~
        +isEffectiveAttacker(Player ai, Card attacker, Combat combat, GameEntity defender) boolean
        +getPossibleBlockers(List~Card~ blockers, List~Card~ attackers, boolean nextTurn) List~Card~
        +canBlockAnAttacker(Card c, List~Card~ attackers, boolean nextTurn) boolean
        +getCardCanBlockAnAttacker(Card c, List~Card~ attackers, boolean nextTurn) Card
        +notNeededAsBlockers(List~Card~ currentAttackers, List~Card~ potentialAttackers) List~Card~
        +reinforceWithBanding(Combat combat) void
        +reinforceWithBanding(Combat combat, Card test) void
        -doAssault() boolean
        -chooseDefender(Combat c, boolean bAssault) GameEntity
        +declareAttackers(Combat combat) int
        +shouldAttack(Card attacker, List~Card~ defenders, Combat combat, GameEntity defender) boolean
        +exertAttackers(List~Card~ attackers, int aggression) List~Card~
        +toProtectAttacker(SpellAbility sa) String
        -doLightmineFieldAttackLogic(Queue~Card~ attackersLeft, int numForcedAttackers, boolean playAggro) void
        -doRevengeOfRavensAttackLogic(GameEntity defender, Queue~Card~ attackersLeft, int numForcedAttackers, Integer maxAttack) boolean
        +countExaltedBonus(Player p) int
        +AiAttackController(Player ai)
        +AiAttackController(Player ai, boolean nextTurn)
        +AiAttackController(Player ai, Card attacker)
    }
    AiAttackController ..> AiController : uses
    AiAttackController ..> Card : uses
    AiAttackController ..> CardCollection : uses
    AiAttackController ..> CardCollectionView : uses
    AiAttackController ..> Combat : uses
    AiAttackController ..> Cost : uses
    AiAttackController ..> FCollection : uses
    AiAttackController ..> FCollectionView : uses
    AiAttackController ..> GameEntity : uses
    AiAttackController ..> GlobalAttackRestrictions : uses
    AiAttackController ..> KeywordInterface : uses
    AiAttackController ..> Player : uses
    AiAttackController ..> PlayerCollection : uses
    AiAttackController ..> PlayerControllerAi : uses
    AiAttackController ..> SpellAbility : uses
    AiAttackController ..> SpellAbilityFactors : uses
    AiAttackController ..> StaticAbility : uses
    AiAttackController ..> Trigger : uses
```

## Relationships
**Uses:**
- [[forge.ai.AiAttackController.SpellAbilityFactors|SpellAbilityFactors]]
