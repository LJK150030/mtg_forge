...
classDiagram
    class DeckFormat {
        ...
        Archenemy
        Puzzle
        -Range~Integer~ mainRange
        -Range~Integer~ sideRange
        -int maxCardCopies
        -Predicate~CardRules~ cardPoolFilter
        -Predicate~PaperCard~ paperCardPoolFilter
        -String ADVPROCLAMATION
        +hasCommander() boolean
        +hasSignatureSpell() boolean
        +smartValueOf(String value, DeckFormat defaultValue) DeckFormat
        +getSideRange() Range~Integer~
        +getMainRange() Range~Integer~
        +getMaxCardCopies() int
        +getMaxCardCopies(PaperCard card) int
        +getExtraSectionMaxCopies(DeckSection section) int
        +getPrimaryDeckSections() EnumSet~DeckSection~
        +getDeckConformanceProblem(Deck deck) String
        +getAttractionDeckConformanceProblem(Deck deck) String
        +getContraptionDeckConformanceProblem(Deck deck) String
        +canHaveAnyNumberOf(IPaperCard iCard) boolean
        +canHaveSpecificNumberInDeck(IPaperCard card) Integer
        +getPlaneSectionConformanceProblem(CardPool planes) String
        +getSchemeSectionConformanceProblem(CardPool schemes) String
        +getCardPool(IDeckGenPool basePool) IDeckGenPool
        +adjustCMCLevels(List~ImmutablePair~ cmcLevels) void
        +allowCustomCards() boolean
        +isLegalCard(PaperCard pc) boolean
        +isLegalCommander(CardRules rules) boolean
        +isLegalDeckPredicate() Predicate~Deck~
        +hasLegalCardsPredicate(boolean enforceDeckLegality) Predicate~Deck~
        +isLegalCardPredicate() Predicate~PaperCard~
        +isLegalCommanderPredicate() Predicate~PaperCard~
        +isLegalCardForCommanderPredicate(List~PaperCard~ commanders) Predicate~PaperCard~
    ...
    DeckFormat ..> CardPool : uses
    DeckFormat ..> CardRules : uses
    DeckFormat ..> ColorSet : uses
    DeckFormat ..> Deck : uses
    DeckFormat ..> DeckGenPool : uses
    DeckFormat ..> DeckSection : uses
    DeckFormat ..> FilterCMC : uses
    DeckFormat ..> ICardFace : uses
    DeckFormat ..> IDeckGenPool : uses
    DeckFormat ..> IPaperCard : uses
    DeckFormat ..> PaperCard : uses
```
...
`DeckFormat` is a `forge-core` enumeration whose constants model the deck-construction rules of each supported Magic format (Constructed, Commander, Limited, Brawl, Pauper, and so on). Each constant stores its own main- and sideboard-size ranges, maximum card-copy count, and optional `CardRules`/`PaperCard` pool predicates, so format-specific constraints live with the value rather than in branching logic. Its core responsibility is deck legality: `getDeckConformanceProblem` plus the attraction, contraption, plane, and scheme checks return human-readable problem strings, mirrored by predicate factories (`isLegalDeckPredicate`, `isLegalCardPredicate`, `isLegalCommanderPredicate`) for filtering and generation.

Collaborating with `Deck`, `CardPool`, `DeckSection`, `CardRules`, `PaperCard`, and `ColorSet`, it enforces singleton, copy-limit, commander color-identity, and partnership rules. Notably, it uses constant-specific class bodies (Limited, TinyLeaders, Adventure) to override defaults and defers volatile commander/variant predicates to `StaticData`, keeping each format's quirks localized.
