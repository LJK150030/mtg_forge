...
classDiagram
    class StaticEffect {
        ...
        -CardCollectionView affectedCards
        -List~Player~ affectedPlayers
        -long timestamp
        -Map~String,String~ mapParams
        -makeMappedCopy(IEntityMap map) StaticEffect
        +setTimestamp(long t) void
        +getTimestamp() long
        +getSource() Card
        +getAffectedCards() CardCollectionView
        +setAffectedCards(CardCollectionView list) void
        +getAffectedPlayers() List~Player~
        +setAffectedPlayers(List~Player~ list) void
        +setParams(Map~String,String~ params) void
        +getParams() Map~String,String~
        +hasParam(String key) boolean
        +getParam(String key) String
        ~remove(Map~StaticAbilityLayer,Set~ affectedPerLayer) CardCollectionView
        ~remove(Map~StaticAbilityLayer,Set~ affectedPerLayer, List~StaticAbilityLayer~ layers) CardCollectionView
        #addCard(Map~StaticAbilityLayer,Set~ affectedByLayer, StaticAbilityLayer layer, Card affectedCard) void
        +removeMapped(IEntityMap map) void
        ~StaticEffect(Card source)
        ~StaticEffect(StaticAbility ability)
    }
    StaticEffect ..> Card : uses
    StaticEffect ..> CardCollection : uses
    StaticEffect ..> CardCollectionView : uses
    StaticEffect ..> IEntityMap : uses
    StaticEffect ..> Player : uses
    StaticEffect ..> StaticAbility : uses
    StaticEffect ..> StaticAbilityLayer : uses
```
