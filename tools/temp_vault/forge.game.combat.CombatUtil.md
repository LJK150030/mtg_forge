...
fqn: forge.game.combat.CombatUtil
...
classDiagram
    class CombatUtil {
        +getAllPossibleDefenders(Player playerWhoAttacks) FCollectionView~GameEntity~
        +validateAttackers(Combat combat) boolean
        +couldAttackButNotAttacking(Combat combat, Card attacker) boolean
        +canAttack(Player p) boolean
        +getPossibleAttackers(Player p) CardCollection
        +canAttack(Card attacker) boolean
        +canAttack(Card attacker, GameEntity defender) boolean
        +canAttackNextTurn(Card attacker, GameEntity defender) boolean
        -canAttack(Card attacker, GameEntity defender, boolean forNextTurn) boolean
        +isAttackerSick(Card attacker, GameEntity defender) boolean
        +checkPropagandaEffects(Game game, Card attacker, Combat combat, List~Card~ attackersWithOptionalCost) boolean
        +getAttackCost(Game game, Card attacker, GameEntity defender) Cost
        +getAttackCost(Game game, Card attacker, GameEntity defender, List~Card~ attackersWithOptionalCost) Cost
        +getOptionalAttackCostCreatures(CardCollection attackers, Class~CostPart~ costType) CardCollection
        +payRequiredBlockCosts(Game game, Card blocker, Card attacker) boolean
        +getBlockCost(Game game, Card blocker, Card attacker) Cost
        +checkDeclaredAttacker(Game game, Card c, Combat combat, boolean triggers) void
        +getAllRequirements(Combat combat) AttackConstraints
        +canBlock(Card blocker, Combat combat) boolean
        +canBlock(Card blocker) boolean
        +canBlock(Card blocker, boolean nextTurn) boolean
        +canBlockMoreCreatures(Card blocker, CardCollectionView blockedBy) boolean
        +canBeBlocked(Card attacker, Combat combat, Player defendingPlayer) boolean
        +canBlockAtLeastOne(Card blocker, Iterable~Card~ attackers) boolean
        +canBeBlocked(Card attacker, List~Card~ blockers, Combat combat) boolean
        +getPotentialBestBlockers(Card attacker, List~Card~ blockers, Combat combat) List~Card~
        +findFreeBlockers(List~Card~ defendersArmy, Combat combat) List~Card~
        +validateBlocks(Combat combat, Player defending) String
        +mustBlockAnAttacker(Card blocker, Combat combat, List~Card~ freeBlockers) boolean
        -attackerLureSatisfied(Card attacker, Card blocker, CardCollection blockers) boolean
        +canBlock(Player p, Combat combat) boolean
        +canBlock(Card attacker, Card blocker, Combat combat) boolean
        +canBlock(Card attacker, Card blocker) boolean
        +canBlock(Card attacker, Card blocker, boolean nextTurn) boolean
        +canAttackerBeBlockedWithAmount(Card attacker, int amount, Combat combat) boolean
        +canAttackerBeBlockedWithAmount(Card attacker, int amount, Player defender) boolean
        +getMinNumBlockersForAttacker(Card attacker, Player defender) int
    }
    CombatUtil ..> AbilityKey : uses
    CombatUtil ..> AttackConstraints : uses
    CombatUtil ..> Card : uses
    CombatUtil ..> CardCollection : uses
    CombatUtil ..> CardCollectionView : uses
    CombatUtil ..> Combat : uses
    CombatUtil ..> Cost : uses
    CombatUtil ..> CostPart : uses
    CombatUtil ..> EmptySa : uses
    CombatUtil ..> FCollection : uses
    CombatUtil ..> FCollectionView : uses
    CombatUtil ..> Game : uses
    CombatUtil ..> GameEntity : uses
    CombatUtil ..> KeywordInterface : uses
    CombatUtil ..> Player : uses
    CombatUtil ..> SpellAbility : uses
    CombatUtil ..> StaticAbility : uses
```
