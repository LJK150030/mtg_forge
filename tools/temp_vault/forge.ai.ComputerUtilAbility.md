...
tags:
  ...
  - module/forge-ai
  - pkg/forge/ai
fqn: forge.ai.ComputerUtilAbility
package: forge.ai
module: forge-ai
kind: Class
---
...
classDiagram
    class ComputerUtilAbility {
        +saComparator saEvaluator
        +getAvailableLandsToPlay(Game game, Player player) CardCollection
        +getAvailableCards(Game game, Player player) CardCollection
        +getSpellAbilities(CardCollectionView all, Player activator) List~SpellAbility~
        +getOriginalAndAltCostAbilities(List~SpellAbility~ originList, Player activator) List~SpellAbility~
        +getTopSpellAbilityOnStack(Game game, SpellAbility sa) SpellAbility
        +getFirstCopySASpell(List~SpellAbility~ spells) SpellAbility
        +getAbilitySource(SpellAbility sa) Card
        +getAbilitySourceName(SpellAbility sa) String
        +getCardsTargetedWithApi(Player ai, CardCollection cardList, SpellAbility sa, ApiType api) CardCollection
        +isFullyTargetable(SpellAbility sa) boolean
        +sortCreatureSpells(List~SpellAbility~ all) List~SpellAbility~
    }
    ComputerUtilAbility ..> ApiType : uses
    ComputerUtilAbility ..> Card : uses
    ComputerUtilAbility ..> CardCollection : uses
    ComputerUtilAbility ..> CardCollectionView : uses
    ComputerUtilAbility ..> CostPart : uses
    ComputerUtilAbility ..> CostPayEnergy : uses
    ComputerUtilAbility ..> CostPutCounter : uses
    ComputerUtilAbility ..> CostRemoveCounter : uses
    ComputerUtilAbility ..> Game : uses
    ComputerUtilAbility ..> OptionalCostValue : uses
    ComputerUtilAbility ..> Player : uses
    ComputerUtilAbility ..> PlayerControllerAi : uses
    ComputerUtilAbility ..> SpellAbility : uses
    ComputerUtilAbility ..> SpellAbilityStackInstance : uses
    ComputerUtilAbility ..> StaticAbility : uses
    ComputerUtilAbility ..> StaticAbilityMode : uses
    ComputerUtilAbility ..> Trigger : uses
    ComputerUtilAbility ..> TriggerType : uses
    ComputerUtilAbility ..> saComparator : uses
```

## Relationships
**Uses:**
- [[forge.ai.ComputerUtilAbility.saComparator|saComparator]]
