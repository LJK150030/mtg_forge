...
## Python
`forge/ai/AiPlayerPredicates.py`
...
class AiPlayerPredicates:
    ...
    @staticmethod
    def compareByZoneValue(type: str, zone: ZoneType, sa: SpellAbility):
        def comparator(arg0, arg1):
            list0 = AbilityUtils.filterListByType(arg0.getCardsIn(zone), type, sa)
            list1 = AbilityUtils.filterListByType(arg1.getCardsIn(zone), type, sa)

            if (CardLists.getNotType(list0, "Creature").isEmpty()
                    and CardLists.getNotType(list1, "Creature").isEmpty()):
                v0 = ComputerUtilCard.evaluateCreatureList(list0)
                v1 = ComputerUtilCard.evaluateCreatureList(list1)
            # otherwise evaluate both lists by CMC and pass only if human
            # permanents are less valuable
    ...
    compareByBoardPosition = staticmethod(
        lambda p: ComputerUtil.evaluateBoardPosition(None, p)
    )
```
