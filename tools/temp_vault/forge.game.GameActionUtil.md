...
classDiagram
    class GameActionUtil {
        +getAlternativeCosts(SpellAbility sa, Player activator, boolean altCostOnly) List~SpellAbility~
        +getGraveyardSpellByKeyword(KeywordInterface inst, SpellAbility sa, Player activator, AlternativeCost altCost) SpellAbility
        +getMayPlaySpellOptions(SpellAbility sa, Card source, Player activator, boolean altCostOnly) List~SpellAbility~
        +getOptionalCostValues(SpellAbility sa) List~OptionalCostValue~
        +addOptionalCosts(SpellAbility sa, List~OptionalCostValue~ list) SpellAbility
        +getAdditionalCostSpell(SpellAbility sa) List~SpellAbility~
        +addExtraKeywordCost(SpellAbility sa) SpellAbility
        +createETBCountersEffect(Card sourceCard, Card c, Player controller, String counter, String amount) Card
        +generatedTotalMana(SpellAbility sa) String
        +generatedMana(SpellAbility sa) String
        +orderCardsByTheirOwners(Game game, CardCollectionView list, ZoneType dest, SpellAbility sa) CardCollectionView
        +rollbackAbility(SpellAbility ability, Zone fromZone, int zonePosition, CostPayment payment, Card oldCard) void
        -GameActionUtil()
    }
    GameActionUtil ..> AbilityManaPart : uses
    GameActionUtil ..> AlternativeCost : uses
    GameActionUtil ..> Card : uses
    GameActionUtil ..> CardCollection : uses
    GameActionUtil ..> CardCollectionView : uses
    GameActionUtil ..> CardPlayOption : uses
    GameActionUtil ..> Cost : uses
    GameActionUtil ..> CostPayment : uses
    GameActionUtil ..> DetachedCardEffect : uses
    GameActionUtil ..> Game : uses
    GameActionUtil ..> KeywordInterface : uses
    GameActionUtil ..> OptionalCost : uses
    GameActionUtil ..> OptionalCostValue : uses
    GameActionUtil ..> Player : uses
    GameActionUtil ..> PlayerCollection : uses
    GameActionUtil ..> PlayerController : uses
    GameActionUtil ..> ReplacementEffect : uses
    GameActionUtil ..> SpellAbility : uses
    GameActionUtil ..> SpellAbilityRestriction : uses
    GameActionUtil ..> StaticAbility : uses
    GameActionUtil ..> Zone : uses
    GameActionUtil ..> ZoneType : uses
```
