...
fqn: forge.game.spellability.SpellAbility
...
classDiagram
    class SpellAbility {
        ...
        -Pair~Long,Player~ controlledByPlayer
        -ManaCostBeingPaid manaCostBeingPaid
        -int spentPhyrexian
        -int paidLifeAmount
        -SpellAbility grantorOriginal
        -StaticAbility grantorStatic
        -CardCollection splicedCards
        -boolean basicSpell
        -Trigger triggerObj
        -boolean optionalTrigger
        -ReplacementEffect replacementEffect
        -AlternativeCost altCost
        -EnumSet~OptionalCost~ optionalCosts
        -Table~Keyword,Pair,Integer~ optionalKeywordAmount
        -boolean aftermath
        -boolean skip
        -Cost payCosts
        -SpellAbilityRestriction restrictions
        -SpellAbilityCondition conditions
        -AbilitySub subAbility
        -Map~String,SpellAbility~ additionalAbilities
        -Map~String,List~ additionalAbilityLists
        #ApiType api
        -List~Mana~ payingMana
        -List~SpellAbility~ paidAbilities
        -Integer xManaCostPaid
        -TreeBasedTable~String,Boolean,CardCollection~ paidLists
        -EnumMap~AbilityKey,Object~ triggeringObjects
        -EnumMap~AbilityKey,Object~ replacingObjects
        -Supplier~List~ pipsToReduce
        -List~AbilitySub~ chosenList
        -Supplier~CardCollection~ tappedForConvoke
        -Card sacrificedAsOffering
        -Card sacrificedAsEmerge
        -Integer maxWaterbend
        -AbilityManaPart manaPart
        -boolean undoable
        -boolean isCopied
        -boolean mayChooseNewTargets
        -boolean isCastFromPlayEffect
        -TargetRestrictions targetRestrictions
        -TargetChoices targetChosen
        -Integer dividedValue
        -SpellAbilityView view
        -CardPlayOption mayPlay
        -CardCollection lastStateBattlefield
        -CardCollection lastStateGraveyard
        -Supplier~CardCollection~ rollbackEffects
        -CardDamageMap damageMap
        -CardDamageMap preventMap
        -GameEntityCounterTable counterTable
        -CardZoneTable changeZoneTable
        -Map~Player,Integer~ loseLifeMap
        -String name
        -nextId() int
        +getLastStateBattlefield() CardCollection
        +setLastStateBattlefield(CardCollectionView lastStateBattlefield) void
        +getLastStateGraveyard() CardCollection
        +setLastStateGraveyard(CardCollectionView lastStateGraveyard) void
        +clearLastState() void
        +getId() int
        +hashCode() int
        ...
        +getApi() ApiType
        +setApi(ApiType apiType) void
        +findSubAbilityByType(ApiType apiType) SpellAbility
        +canThisProduce(String s) boolean
        +canProduce(String s) boolean
        +isManaAbilityFor(SpellAbility saPaidFor, byte colorNeeded) boolean
        +isManaCannotCounter(SpellAbility saPaidFor) boolean
        +amountOfManaGenerated(boolean multiply) int
        +totalAmountOfManaGenerated(SpellAbility saPaidFor, boolean multiply) int
        +setManaExpressChoice(ColorSet cs) void
        +getManaPart() AbilityManaPart
        +getAllManaParts() List~AbilityManaPart~
        +isManaAbility() boolean
        +setManaPart(AbilityManaPart manaPart0) void
        +allowsPayingWithShard(Card src, byte shard) boolean
        +cantBeCopied() boolean
        +canPlay() boolean
        +canPlay(boolean checkOptionalCosts) boolean
        +canPlayWithOptionalCost(OptionalCostValue opt) boolean
        +isPossible() boolean
        +promptIfOnlyPossibleAbility() boolean
        +resolve() void
        +getActivatingPlayer() Player
        +setActivatingPlayer(Player player) void
        +getTargetingPlayer() Player
        +setTargetingPlayer(Player targetingPlayer0) void
        +getChoosingPlayer() Player
        +setChoosingPlayer(Player choosingPlayer0) void
        +getControlledByPlayer() Pair~Long,Player~
        +setControlledByPlayer(long ts, Player controller) void
        +getManaCostBeingPaid() ManaCostBeingPaid
        +setManaCostBeingPaid(ManaCostBeingPaid costBeingPaid) void
        +getOptionalCosts() Iterable~OptionalCost~
        +addOptionalCost(OptionalCost cost) void
        +isOptionalCostPaid(OptionalCost cost) boolean
        +isSpell() boolean
        +isAbility() boolean
        +isActivatedAbility() boolean
        +isLandAbility() boolean
        +isTurnFaceUp() boolean
        +isMorphUp() boolean
        +isDisguiseUp() boolean
        +isCastFaceDown() boolean
        +isManifestUp() boolean
        +isCloakUp() boolean
        +isUnlock() boolean
        +isCycling() boolean
        +isBackup() boolean
        ...
        +isEquip() boolean
        +isAftermath() boolean
        +setAftermath(boolean aftermath) void
        +isChapter() boolean
        +getChapter() Integer
        ...
        +getResolvedThisTurn() int
        +getConditions() SpellAbilityCondition
        +setConditions(SpellAbilityCondition condition) void
        +metConditions() boolean
        +getPayingMana() List~Mana~
        +clearManaPaid() void
        +getSpendPhyrexianMana() int
        +setSpendPhyrexianMana(boolean inc) void
        +getAmountLifePaid() int
        +setPaidLife(int value) void
        +applyPayingManaEffects() void
        +getPayingColors() ColorSet
        +getPayingManaAbilities() List~SpellAbility~
        ...
        +getPaidList(String str) Iterable~Card~
        ...
        +setTriggeringObjectsFrom(Map~AbilityKey,Object~ runParams, AbilityKey types) void
        +hasTriggeringObject(AbilityKey type) boolean
        +resetTriggeringObjects() void
        +getReplacingObjects() Map~AbilityKey,Object~
        +getReplacingObject(AbilityKey type) Object
        +setReplacingObject(AbilityKey type, Object o) void
        +setReplacingObjects(Map~AbilityKey,Object~ repParams) void
        +setReplacingObjectsFrom(Map~AbilityKey,Object~ repParams, AbilityKey types) void
        +resetOnceResolved() void
        +yieldKey() String
        +getStackDescription() String
        +setStackDescription(String s) void
        +getOriginalStackDescription() String
        +getDescription() String
        +setDescription(String s) void
        +getOriginalDescription() String
        +getCostDescription() String
        +rebuiltDescription() void
        +toString() String
        +toUnsuppressedString() String
        +getSubAbility() AbilitySub
        +setSubAbility(AbilitySub subAbility0) void
        +getAdditionalAbilities() Map~String,SpellAbility~
        +getAdditionalAbility(String name) SpellAbility
        +hasAdditionalAbility(String name) boolean
        +setAdditionalAbility(String name, SpellAbility sa) void
        ...
        +appendSubAbility(AbilitySub toAdd) void
        +isBasicSpell() boolean
        +setBasicSpell(boolean basicSpell0) void
        +getMayPlayOption() CardPlayOption
        +getMayPlay() StaticAbility
        +setMayPlay(CardPlayOption sta) void
        +copy() SpellAbility
        +copy(Player activ) SpellAbility
        +copy(Card host, boolean lki) SpellAbility
        +copy(Card host, boolean lki, boolean keepTextChanges) SpellAbility
        +copy(Card host, Player activ, boolean lki) SpellAbility
        +copy(Card host, Player activ, boolean lki, boolean keepTextChanges) SpellAbility
        +copyWithNoManaCost() SpellAbility
        +copyWithNoManaCost(Player active) SpellAbility
        +copyWithDefinedCost(Cost abCost) SpellAbility
        +copyWithManaCostReplaced(Player active, Cost abCost) SpellAbility
        +isTrigger() boolean
        +getTrigger() Trigger
        +setTrigger(Trigger t) void
        +isOptionalTrigger() boolean
        +setOptionalTrigger(boolean optrigger) void
        +getSourceTrigger() int
        +isReplacementAbility() boolean
        +getReplacementEffect() ReplacementEffect
        +setReplacementEffect(ReplacementEffect re) void
        +isMandatory() boolean
        +canTarget(GameObject entity) boolean
        +canTarget(GameObject entity, boolean fizzleCheck) boolean
        +isWrapper() boolean
        +getPipsToReduce() List~String~
        +clearPipsToReduce() void
        +getTappedForConvoke() CardCollection
        +addTappedForConvoke(Card c) void
        +clearTappedForConvoke() void
        +isEmerge() boolean
        +getSacrificedAsEmerge() Card
        +setSacrificedAsEmerge(Card c) void
        +resetSacrificedAsEmerge() void
        +isOffering() boolean
        +getSacrificedAsOffering() Card
        +setSacrificedAsOffering(Card c) void
        +resetSacrificedAsOffering() void
        +getSplicedCards() CardCollection
        +addSplicedCards(Card splicedCard) void
        +knownDetermineDefined(String defined) CardCollection
        +getRootAbility() SpellAbility
        +getTailAbility() SpellAbility
        +getParent() SpellAbility
        #getSVarFallback(String name) List~IHasSVars~
        +isUndoable() boolean
        +undo() boolean
        +setUndoable(boolean b) void
        +isCastFromPlayEffect() boolean
        +setCastFromPlayEffect(boolean b) void
        +isCopied() boolean
        +setCopied(boolean isCopied0) void
        +isMayChooseNewTargets() boolean
        +setMayChooseNewTargets(boolean value) void
        +isAnnouncing(String variable) boolean
        +addAnnounceVar(String variable) void
        ...
        +getDividedValue() Integer
        +getStillToDivide() int
        +resetFirstTarget(GameObject c, SpellAbility originalSA) void
        +canAddMoreTarget() boolean
        +isZeroTargets() boolean
        +isMinTargetChosen() boolean
        +isMaxTargetChosen() boolean
        +getMinTargets() int
        +getMaxTargets() int
        +isTargetNumberValid() boolean
        +getAllTargetChoices() List~TargetChoices~
        +getTargetCard() Card
        +setTargetCard(Card card) void
        +findTargetedCards() CardCollectionView
        +getSATargetingCard() SpellAbility
        +getParentTargetingCard() SpellAbility
        +getSATargetingSA() SpellAbility
        +getParentTargetingSA() SpellAbility
        +getSATargetingPlayer() SpellAbility
        +getParentTargetingPlayer() SpellAbility
        +getUniqueTargets() List~GameObject~
        +canTargetSpellAbility(SpellAbility topSA) boolean
        +isTargeting(GameObject o) boolean
        +setupNewTargets(Player forceTargetingPlayer) boolean
        +setupTargets() boolean
        +clearTargets() void
        ...
        +tracksManaSpent() boolean
        +getTotalManaSpent() int
        ...
        +getView() SpellAbilityView
        +compareTo(SpellAbility ab) int
        +calculateScoreForManaAbility() int
        ...
        +getLoseLifeMap() Map~Player,Integer~
        ...
        +setLoseLifeMap(Map~Player,Integer~ map) void
        +getOriginalAbility() SpellAbility
        +setOriginalAbility(SpellAbility sa) void
        +getGrantorStatic() StaticAbility
        +setGrantorStatic(StaticAbility st) void
        ...
        +getXColor() String
        +isSkip() boolean
        +setSkip(boolean val) void
        +canCastTiming(Player activator) boolean
        +canCastTiming(Card host, Player activator) boolean
        +withFlash(Card host, Player activator) boolean
        +checkRestrictions(Player activator) boolean
        +checkRestrictions(Card host, Player activator) boolean
        +addRollbackEffect(Card eff) void
        +rollback() void
        +isHidden() boolean
        +isLegalAfterStack() boolean
        +isCounterableBy(SpellAbility sa) boolean
        +getAlternateHost(Card source) Card
        +hasOptionalKeywordAmount(KeywordInterface kw) boolean
        +hasOptionalKeywordAmount(Keyword kw) boolean
        +getOptionalKeywords() Set~Keyword~
        +getOptionalKeywordAmount(KeywordInterface kw) int
        +getOptionalKeywordAmount(Keyword kw) int
        +setOptionalKeywordAmount(KeywordInterface kw, int amount) void
        +clearOptionalKeywordAmount() void
        +getName() String
        +setName(String name) void
        +getMaxWaterbend() Integer
        +setMaxWaterbend(Cost cost) void
        #SpellAbility(Card iSourceCard, Cost toPay)
        #SpellAbility(Card iSourceCard, Cost toPay, SpellAbilityView view0)
        #SpellAbility(Card iSourceCard, Cost toPay, SpellAbilityView view0, CardState cs)
    }
    SpellAbility --|> CardTraitBase : extends
    SpellAbility ..|> ISpellAbility : implements
    SpellAbility ..|> IIdentifiable : implements
    SpellAbility ..|> Comparable : implements
    SpellAbility ..> AbilityKey : uses
    SpellAbility ..> AbilityManaPart : uses
    SpellAbility ..> AbilityStatic : uses
    SpellAbility ..> AbilitySub : uses
    SpellAbility ..> AlternativeCost : uses
    SpellAbility ..> ApiType : uses
    SpellAbility ..> Card : uses
    SpellAbility ..> CardCollection : uses
    SpellAbility ..> CardCollectionView : uses
    SpellAbility ..> CardDamageMap : uses
    SpellAbility ..> CardPlayOption : uses
    SpellAbility ..> CardState : uses
    SpellAbility ..> CardZoneTable : uses
    SpellAbility ..> ColorSet : uses
    SpellAbility ..> Cost : uses
    SpellAbility ..> CostPart : uses
    SpellAbility ..> CostSacrifice : uses
    SpellAbility ..> CostTap : uses
    SpellAbility ..> Game : uses
    SpellAbility ..> GameCommand : uses
    SpellAbility ..> GameEntity : uses
    SpellAbility ..> GameEntityCounterTable : uses
    SpellAbility ..> GameEventCardStatsChanged : uses
    SpellAbility ..> GameObject : uses
    SpellAbility ..> IHasSVars : uses
    SpellAbility ..> ITranslatable : uses
    SpellAbility ..> Keyword : uses
    SpellAbility ..> KeywordInterface : uses
    SpellAbility ..> Mana : uses
    SpellAbility ..> ManaCostBeingPaid : uses
    SpellAbility ..> OptionalCost : uses
    SpellAbility ..> OptionalCostValue : uses
    SpellAbility ..> Player : uses
    SpellAbility ..> PlayerCollection : uses
    SpellAbility ..> ReplacementEffect : uses
    SpellAbility ..> SpellAbilityCondition : uses
    SpellAbility ..> SpellAbilityRestriction : uses
    SpellAbility ..> SpellAbilityView : uses
    SpellAbility ..> StaticAbility : uses
    SpellAbility ..> TargetChoices : uses
    SpellAbility ..> TargetRestrictions : uses
    SpellAbility ..> Trigger : uses
    SpellAbility ..> WrappedAbility : uses
```
...
- [[forge.game.spellability.ISpellAbility|ISpellAbility]]
**Uses:**
