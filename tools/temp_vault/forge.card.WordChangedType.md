...
The record form signals that the rule itself is a value objectâ€”two final words defining the transformationâ€”while the actual state change is delegated to the passed-in `CardType` collaborator. The guard against `hasStringType` keeps the operation a safe no-op when the target word is absent, allowing such rules to be applied indiscriminately across cards without side effects on non-matching types.
...
class WordChangedType(ICardChangedType):
    def __init__(self, oldWord: str, newWord: str):
        self._oldWord = oldWord
        self._newWord = newWord

    def oldWord(self) -> str:
        return self._oldWord

    def newWord(self) -> str:
        return self._newWord

    def applyChanges(self, newType: CardType) -> CardType:
        if newType.hasStringType(self._oldWord):
            newType.subtypes.remove(self._oldWord)
            newType.subtypes.add(self._newWord)
        return newType
```
