...
tags:
  ...
  - module/forge-game
  - pkg/forge/game/card
fqn: forge.game.card.CardFactoryUtil
...
```mermaid
classDiagram
    class CardFactoryUtil {
        +abilityCastFaceDown(CardState cardState, boolean intrinsic, String key) SpellAbility
        +abilityUnlockRoom(CardState cardState) SpellAbility
        +abilityTurnFaceUp(CardState cardState, Cost cost, String key, String desc, String reminderDesc) SpellAbility
        +handleHiddenAgenda(Player player, Card card, KeywordInterface ki) boolean
        -abilityRevealHiddenAgenda(Card sourceCard) SpellAbility
        +extractOperators(String expression) String
        +getMostProminentColors(Iterable~Card~ list) byte
        +SortColorsFromList(CardCollection list) int[]
        +getMostProminentColorsFromList(CardCollectionView list, Iterable~String~ restrictedToColors) byte
        +getMostProminentCreatureTypeSize(CardCollection list) int
        +getMostProminentCreatureType(CardCollectionView list) Iterable~String~
        +sharedKeywords(Iterable~String~ kw, String[] restrictions, Iterable~ZoneType~ zones, Card host, CardTraitBase ctb) List~String~
        +getSharedKeywords(Iterable~String~ kw, CardCollection cards) List~String~
        +addAbilityFactoryAbilities(Card card, Iterable~String~ abilities) void
        +setupKeywordedAbilities(Card card) void
        -createETBReplacement(CardState card, ReplacementLayer layer, String effect, boolean optional, boolean secondary, boolean intrinsic, String valid, String zone) ReplacementEffect
        -createETBReplacement(CardState card, ReplacementLayer layer, SpellAbility repAb, boolean optional, boolean secondary, boolean intrinsic, String valid, String zone) ReplacementEffect
        +getProtectionValid(String kw, boolean damage) String
        +makeEtbCounter(String kw, CardState card, boolean intrinsic) ReplacementEffect
        +addTriggerAbility(KeywordInterface inst, Card card, boolean intrinsic) void
        +addReplacementEffect(KeywordInterface inst, CardState card, boolean intrinsic) void
        +addSpellAbility(KeywordInterface inst, CardState card, boolean intrinsic) void
        +addStaticAbility(KeywordInterface inst, CardState state, boolean intrinsic) void
        +setupSiegeAbilities(Card card) void
        +setupAdventureAbility(CardState card) ReplacementEffect
        +setupOmenAbility(CardState card) ReplacementEffect
        +setFaceDownState(Card c, SpellAbility sa) void
    }
    CardFactoryUtil ..> AbilityKey : uses
    CardFactoryUtil ..> AbilityStatic : uses
    CardFactoryUtil ..> AbilitySub : uses
    CardFactoryUtil ..> Affinity : uses
    CardFactoryUtil ..> Card : uses
    CardFactoryUtil ..> CardCollection : uses
    CardFactoryUtil ..> CardCollectionView : uses
    CardFactoryUtil ..> CardState : uses
    CardFactoryUtil ..> CardStateName : uses
    CardFactoryUtil ..> CardTraitBase : uses
    CardFactoryUtil ..> CardType : uses
    CardFactoryUtil ..> CardTypeView : uses
    CardFactoryUtil ..> CardZoneTable : uses
    CardFactoryUtil ..> ColorSet : uses
    CardFactoryUtil ..> Cost : uses
    CardFactoryUtil ..> CostCollectEvidence : uses
    CardFactoryUtil ..> CostPart : uses
    CardFactoryUtil ..> CostPartMana : uses
    CardFactoryUtil ..> CounterType : uses
    CardFactoryUtil ..> Craft : uses
    CardFactoryUtil ..> Devour : uses
    CardFactoryUtil ..> Emerge : uses
    CardFactoryUtil ..> EmptySa : uses
    CardFactoryUtil ..> Game : uses
    CardFactoryUtil ..> GameCommand : uses
    CardFactoryUtil ..> GameEntityCounterTable : uses
    CardFactoryUtil ..> GameEventAddLog : uses
    CardFactoryUtil ..> GameEventCardForetold : uses
    CardFactoryUtil ..> GameEventCardPlotted : uses
    CardFactoryUtil ..> Hexproof : uses
    CardFactoryUtil ..> ICardFace : uses
    CardFactoryUtil ..> KeywordInterface : uses
    CardFactoryUtil ..> KeywordWithCost : uses
    CardFactoryUtil ..> KeywordWithCostAndAmount : uses
    CardFactoryUtil ..> KeywordWithCostAndType : uses
    CardFactoryUtil ..> Landwalk : uses
    CardFactoryUtil ..> Modular : uses
    CardFactoryUtil ..> Player : uses
    CardFactoryUtil ..> ReplacementEffect : uses
    CardFactoryUtil ..> ReplacementLayer : uses
    CardFactoryUtil ..> Spell : uses
    CardFactoryUtil ..> SpellAbility : uses
    CardFactoryUtil ..> SpellPermanent : uses
    CardFactoryUtil ..> StaticAbility : uses
    CardFactoryUtil ..> TargetRestrictions : uses
    CardFactoryUtil ..> TextUtil : uses
    CardFactoryUtil ..> Trigger : uses
    CardFactoryUtil ..> Vanishing : uses
    CardFactoryUtil ..> ZoneType : uses
```
...
- [[forge.game.keyword.Affinity|Affinity]]
- [[forge.game.keyword.Craft|Craft]]
- [[forge.game.keyword.Devour|Devour]]
- [[forge.game.keyword.Emerge|Emerge]]
- [[forge.game.keyword.Hexproof|Hexproof]]
...
- [[forge.game.keyword.Landwalk|Landwalk]]
- [[forge.game.keyword.Modular|Modular]]
...
- [[forge.util.TextUtil|TextUtil]]

## Design Description

CardFactoryUtil is a stateless utility class in `forge.game.card` providing static helpers that translate Magic's textual keywords and abilities into the engine's executable trait objects. Its central responsibility is keyword realization: methods such as `addTriggerAbility`, `addReplacementEffect`, `addSpellAbility`, and `addStaticAbility` parse each `KeywordInterface` and attach the corresponding `Trigger`, `ReplacementEffect`, `SpellAbility`, or `StaticAbility` to a `Card`/`CardState`, almost always by building Forge's script-string DSL and delegating to factories like `AbilityFactory`, `TriggerHandler`, and `ReplacementHandler`. It also offers analytical helpers (most-prominent color/creature-type tallies, shared-keyword resolution) and constructs special play modes (morph/face-down casting, foretell, plot, suspend, adventure/omen, siege).

As a pure helper, it has no supertype and holds no state; it collaborates broadly across the game model rather than owning any. The design intent is to centralize the verbose, error-prone mapping from keyword text to game mechanics in one place, keeping per-card logic data-driven through DSL strings, with Sentry breadcrumbs aiding diagnosis when a card's raw ability fails to parse.
