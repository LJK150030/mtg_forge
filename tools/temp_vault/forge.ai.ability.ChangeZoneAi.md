...
fqn: forge.ai.ability.ChangeZoneAi
...
classDiagram
    class ChangeZoneAi {
        -CardCollection multipleCardsToChoose
        ...
        -hiddenOriginCanPlayAI(Player ai, SpellAbility sa) AiAbilityDecision
        -hiddenOriginPlayDrawbackAI(Player aiPlayer, SpellAbility sa) AiAbilityDecision
        -hiddenTriggerAI(Player ai, SpellAbility sa, boolean mandatory) AiAbilityDecision
        -basicManaFixing(Player ai, List~Card~ list) Card
        -areAllBasics(String types) boolean
        -chooseCreature(Player ai, CardCollection list) Card
        -knownOriginCanPlayAI(Player ai, SpellAbility sa) AiAbilityDecision
        #checkPhaseRestrictions(Player ai, SpellAbility sa, PhaseHandler ph) boolean
        -knownOriginPlayDrawbackAI(Player aiPlayer, SpellAbility sa) AiAbilityDecision
        -isPreferredTarget(Player ai, SpellAbility sa, boolean mandatory, boolean immediately) boolean
        -canBouncePermanent(Player ai, SpellAbility sa, CardCollectionView list) Card
        -isUnpreferredTarget(Player ai, SpellAbility sa, boolean mandatory) boolean
        -knownOriginTriggerAI(Player ai, SpellAbility sa, boolean mandatory) AiAbilityDecision
        +chooseCardToHiddenOriginChangeZone(ZoneType destination, List~ZoneType~ origin, SpellAbility sa, CardCollection fetchList, Player player, Player decider) Card
        -prefilterOwnListForBounceAnyNum(CardCollection fetchList, Player decider) CardCollection
        ...
        -doSacAndReturnFromGraveLogic(Player ai, SpellAbility sa) AiAbilityDecision
        -doSacAndUpgradeLogic(Player ai, SpellAbility sa) AiAbilityDecision
        +doReturnCommanderLogic(SpellAbility sa, Player aiPlayer) AiAbilityDecision
        +doExileCombatThreatLogic(Player aiPlayer, SpellAbility sa) AiAbilityDecision
        +doExilePreferenceLogic(Player aiPlayer, SpellAbility sa, CardCollection fetchList) Card
        -doExileSpellLogic(Player ai, SpellAbility sa, boolean mandatory) boolean
        -getSafeTargetsIfUnlessCostPaid(Player ai, SpellAbility sa, Iterable~Card~ potentialTgts) CardCollection
        -rememberBouncedThisTurn(Player ai, Card c) void
        -isBouncedThisTurn(Player ai, Card c) boolean
        -doBounceOwnTriggerLogic(Player ai, SpellAbility sa, CardCollection choices) Card
        -considerRamp(Player ai, SpellAbility sa, CardCollection choices, Card keycardFound) Card
        +willPayUnlessCost(Player payer, SpellAbility sa, Cost cost, boolean alreadyPaid, FCollectionView~Player~ payers) boolean
    }
    ChangeZoneAi --|> SpellAbilityAi : extends
    ChangeZoneAi ..> AbilityKey : uses
    ChangeZoneAi ..> AbilitySub : uses
    ChangeZoneAi ..> AiAbilityDecision : uses
    ChangeZoneAi ..> ApiType : uses
    ChangeZoneAi ..> Card : uses
    ChangeZoneAi ..> CardCollection : uses
    ChangeZoneAi ..> CardCollectionView : uses
    ChangeZoneAi ..> CardType : uses
    ChangeZoneAi ..> Combat : uses
    ChangeZoneAi ..> CoreType : uses
    ChangeZoneAi ..> Cost : uses
    ChangeZoneAi ..> CostDamage : uses
    ChangeZoneAi ..> CostDiscard : uses
    ChangeZoneAi ..> CostPart : uses
    ChangeZoneAi ..> CostPayLife : uses
    ChangeZoneAi ..> CostPutCounter : uses
    ChangeZoneAi ..> CounterType : uses
    ChangeZoneAi ..> FCollectionView : uses
    ChangeZoneAi ..> Game : uses
    ChangeZoneAi ..> GameEntity : uses
    ChangeZoneAi ..> GameObject : uses
    ChangeZoneAi ..> PhaseHandler : uses
    ChangeZoneAi ..> Player : uses
    ChangeZoneAi ..> PlayerActionConfirmMode : uses
    ChangeZoneAi ..> SpellAbility : uses
    ChangeZoneAi ..> TargetRestrictions : uses
    ChangeZoneAi ..> ZoneType : uses
```
