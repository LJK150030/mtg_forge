...
fqn: forge.ai.simulation.GameStateEvaluator
...
classDiagram
    class GameStateEvaluator {
        -boolean debugging
        -SimulationCreatureEvaluator eval
        +setDebugging(boolean debugging) void
        -debugPrint(String s) void
        -simulateUpcomingCombatThisTurn(Game evalGame, Player aiPlayer) CombatSimResult
        -cardToString(Card c) String
        -getScoreForGameOver(Game game, Player aiPlayer) Score
        +getScoreForGameState(Game game, Player aiPlayer) Score
        -getScoreForGameStateImpl(Game game, Player aiPlayer) Score
        +evalManaBase(Game game, Player player, AiDeckStatistics statistics) int
        +evalCard(Game game, Player aiPlayer, Card c) int
        +evaluateLand(Card c) int
    }
    GameStateEvaluator ..> AbilityManaPart : uses
    GameStateEvaluator ..> AiDeckStatistics : uses
    GameStateEvaluator ..> Card : uses
    GameStateEvaluator ..> CombatSimResult : uses
    GameStateEvaluator ..> CostSacrifice : uses
    GameStateEvaluator ..> CreatureEvaluator : uses
    GameStateEvaluator ..> Game : uses
    GameStateEvaluator ..> GameCopier : uses
    GameStateEvaluator ..> PhaseType : uses
    GameStateEvaluator ..> Player : uses
    GameStateEvaluator ..> Score : uses
    GameStateEvaluator ..> SimulationCreatureEvaluator : uses
    GameStateEvaluator ..> SpellAbility : uses
    GameStateEvaluator ..> StaticAbility : uses
```
...
- [[forge.ai.simulation.GameStateEvaluator.CombatSimResult|CombatSimResult]]
- [[forge.ai.simulation.GameStateEvaluator.Score|Score]]
- [[forge.ai.simulation.GameStateEvaluator.SimulationCreatureEvaluator|SimulationCreatureEvaluator]]
