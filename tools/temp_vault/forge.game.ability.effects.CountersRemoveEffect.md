---
aliases:
  - CountersRemoveEffect
tags:
  - java/class
  - module/forge-game
  - pkg/forge/game/ability/effects
fqn: forge.game.ability.effects.CountersRemoveEffect
...
```mermaid
classDiagram
    class CountersRemoveEffect {
        #getStackDescription(SpellAbility sa) String
        +resolve(SpellAbility sa) void
        #removeAnyType(GameEntity entity, int cntToRemove, SpellAbility sa) int
    }
    CountersRemoveEffect --|> SpellAbilityEffect : extends
    CountersRemoveEffect ..> Card : uses
    CountersRemoveEffect ..> CardCollection : uses
    CountersRemoveEffect ..> CardCollectionView : uses
    CountersRemoveEffect ..> CounterType : uses
    CountersRemoveEffect ..> Game : uses
    CountersRemoveEffect ..> GameEntity : uses
    CountersRemoveEffect ..> Player : uses
    CountersRemoveEffect ..> PlayerController : uses
    CountersRemoveEffect ..> SpellAbility : uses
    CountersRemoveEffect ..> Zone : uses
    CountersRemoveEffect ..> ZoneType : uses
```
...
CountersRemoveEffect is a concrete `SpellAbilityEffect` subclass that resolves the "CountersRemove" game action, removing counters from targeted `Card`s and `Player`s. It overrides `getStackDescription` to build a readable summary and `resolve` to perform the removal, driven entirely by the SpellAbility's parameters (CounterType, CounterNum, UpTo, Choices, Optional, Remember flags). It supports fixed counts, "All", and interactive "Any"/up-to selection delegated to the activator's `PlayerController`.

Notable design intent: the effect re-resolves each target through current `Game`/`Zone` state and compares game timestamps to skip stale LKI cards, guarding against removing counters from objects no longer in play. The private `removeAnyType` helper handles mixed-type removal, prompting for type and quantity while computing a minimum that keeps the requested total reachable across remaining counter types. A running `totalRemoved` tally can be stashed back on the source card via RememberRemoved/RememberAmount for dependent abilities.

## Source
`forge-game/src/main/java/forge/game/ability/effects/CountersRemoveEffect.java`
