...
## Python
`forge/game/ability/effects/CountersNoteEffect.py`
...
class CountersNoteEffect(SpellAbilityEffect):
    # Primarily used for "Noting" Counters on a card. i.e. Oubliette or Tawnos' Coffin
    # Counters get "noted" as SVars onto the Source
    # "Mode": "Store" or "Load"
    MODE_STORE = "Store"
    MODE_LOAD = "Load"
    NOTE_COUNTERS = "NoteCounters"

    def resolve(self, sa: SpellAbility) -> None:
        pass

    @staticmethod
    def noteCounters(notee: Card, source: Card) -> None:
        for counter in notee.getCounters().entrySet():
            sb = []
            sb.append(CountersNoteEffect.NOTE_COUNTERS)
            sb.append(counter.getKey().getName())
            source.setSVar("".join(sb), str(counter.getValue()))

    @staticmethod
    def loadCounters(notee: Card, source: Card, p: Player, sa: SpellAbility, moveParams: dict[AbilityKey, Any]) -> None:
        table = GameEntityCounterTable()
        for svar in source.getSVars().entrySet():
            key = svar.getKey()
            if key.startswith(CountersNoteEffect.NOTE_COUNTERS):
                cType = CounterType.getType(key[len(CountersNoteEffect.NOTE_COUNTERS):])
                cAmount = int(svar.getValue())
                table.put(sa.getActivatingPlayer(), notee, cType, cAmount)
            # TODO Probably should "remove" the svars that were temporarily used
