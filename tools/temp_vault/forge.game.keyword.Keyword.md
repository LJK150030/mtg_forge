...
fqn: forge.game.keyword.Keyword
...
**Package:** `forge.game.keyword` &nbsp; **Module:** `forge-game` &nbsp; **Kind:** Enum
...
classDiagram
    class Keyword {
        ...
        MAYFLASHCOST
        MAYFLASHSAC
        #Class~KeywordInstance~ type
        #boolean isMultipleRedundant
        #String reminderText
        #String displayName
        -Map~String,Set~ cardKeywordSetLookup
        -getKeywordDetails(String k) Pair~Keyword,String~
        +getInstance(String k) KeywordInterface
        +toString() String
        +getAllKeywords() List~Keyword~
        -get(String k) Keyword
        +getKeywordSet(PaperCard card) Set~Keyword~
        +smartValueOf(String value) Keyword
        +setValueOf(String value) Set~Keyword~
        +getReminderText() String
        +isMultipleRedundant() boolean
        ~Keyword(String displayName0, Class~KeywordInstance~ type0, boolean isMultipleRedundant0, String reminderText0)
    }
    Keyword ..> Affinity : uses
    Keyword ..> Amplify : uses
    Keyword ..> CardSplitType : uses
    Keyword ..> Companion : uses
    Keyword ..> Compleated : uses
    Keyword ..> Craft : uses
    Keyword ..> Devour : uses
    Keyword ..> Emerge : uses
    Keyword ..> Equip : uses
    Keyword ..> Firebending : uses
    Keyword ..> Hexproof : uses
    Keyword ..> KeywordInstance : uses
    Keyword ..> KeywordInterface : uses
    Keyword ..> KeywordWithAmount : uses
    Keyword ..> KeywordWithCost : uses
    Keyword ..> KeywordWithCostAndAmount : uses
    Keyword ..> KeywordWithCostAndType : uses
    Keyword ..> KeywordWithType : uses
    Keyword ..> Kicker : uses
    Keyword ..> Landwalk : uses
    Keyword ..> Mayhem : uses
    Keyword ..> Modular : uses
    Keyword ..> Ninjutsu : uses
    Keyword ..> PaperCard : uses
    Keyword ..> Partner : uses
    Keyword ..> Protection : uses
    Keyword ..> SimpleKeyword : uses
    Keyword ..> Suspend : uses
    Keyword ..> Trample : uses
    Keyword ..> Vanishing : uses
```
...
- [[forge.game.keyword.Amplify|Amplify]]
- [[forge.game.keyword.Companion|Companion]]
- [[forge.game.keyword.Compleated|Compleated]]
...
- [[forge.game.keyword.Firebending|Firebending]]
...
- [[forge.game.keyword.Kicker|Kicker]]
- [[forge.game.keyword.Landwalk|Landwalk]]
- [[forge.game.keyword.Mayhem|Mayhem]]
- [[forge.game.keyword.Modular|Modular]]
- [[forge.game.keyword.Ninjutsu|Ninjutsu]]
- [[forge.game.keyword.Partner|Partner]]
- [[forge.game.keyword.Protection|Protection]]
- [[forge.game.keyword.SimpleKeyword|SimpleKeyword]]
- [[forge.game.keyword.Suspend|Suspend]]
- [[forge.game.keyword.Trample|Trample]]
