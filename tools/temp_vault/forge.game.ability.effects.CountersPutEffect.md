...
fqn: forge.game.ability.effects.CountersPutEffect
...
```mermaid
classDiagram
    class CountersPutEffect {
        #getStackDescription(SpellAbility sa) String
        #resolvePerType(SpellAbility sa, Player placer, CounterType counterType, int counterAmount, GameEntityCounterTable table, boolean stopForTypes) void
        +resolve(SpellAbility sa) void
        #addRemovePhaseTrigger(Card host, SpellAbility sa, String phase, GameEntity tgt, CounterType ct, int added) void
        #chooseTypeFromList(SpellAbility sa, String list, GameEntity obj, PlayerController pc) CounterType
        #logOutput(Map~Object,Integer~ randomMap, Card card) String
        +buildSpellAbility(SpellAbility sa) void
    }
    CountersPutEffect --|> SpellAbilityEffect : extends
    CountersPutEffect ..> AbilityKey : uses
    CountersPutEffect ..> Card : uses
    CountersPutEffect ..> CardCollection : uses
    CountersPutEffect ..> CardCollectionView : uses
    CountersPutEffect ..> CounterType : uses
    CountersPutEffect ..> Game : uses
    CountersPutEffect ..> GameEntity : uses
    CountersPutEffect ..> GameEntityCounterTable : uses
    CountersPutEffect ..> GameEventRandomLog : uses
    CountersPutEffect ..> Player : uses
    CountersPutEffect ..> PlayerController : uses
    CountersPutEffect ..> SpellAbility : uses
    CountersPutEffect ..> TargetRestrictions : uses
    CountersPutEffect ..> Trigger : uses
    CountersPutEffect ..> ZoneType : uses
```
...
CountersPutEffect implements the resolution logic for any spell or ability that places counters on game entities, acting as the counter-adding member of Forge's effect hierarchy. As a concrete subclass of SpellAbilityEffect, it overrides `resolve` to read the host SpellAbility's parameters and `getStackDescription` to render readable stack text, delegating the actual placement to the protected `resolvePerType` helper. It operates over Card and Player targets (as GameEntity), funneling all additions through a single GameEntityCounterTable so they are batched and processed through replacement effects in one `replaceCounterEffect` pass.

The class deliberately concentrates wide variation behind one effect: divided, random, "up to," existing-type, per-defined, and shared-keyword distributions, plus the Adapt, Bolster, Monstrosity, Support, Tribute, Renown, and Mentor mechanics, which `buildSpellAbility` normalizes into ordinary counter parameters. Notable design intent includes guarding against stale targets via game-state and timestamp re-checks, deferring ETB counters to a replacement-supplied table, firing keyword triggers, and registering per-counter delayed triggers for temporary "remove next phase" counters.
