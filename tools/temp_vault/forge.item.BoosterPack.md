...
classDiagram
    class BoosterPack {
        -int artIndex
        -int hash
        +fromSet(CardEdition edition) BoosterPack
        +fromColor(String color) BoosterPack
        +getArtIndex() int
        +getItemType() String
        +getDescription() String
        +clone() Object
        +getBoosterData() SealedTemplate
        +equals(Object o) boolean
        +hashCode() int
        +getImageKey(boolean altState) String
        +BoosterPack(String name0, SealedTemplate boosterData)
    }
    BoosterPack --|> SealedProduct : extends
    BoosterPack ..> CardEdition : uses
    BoosterPack ..> SealedTemplate : uses
```
