...
fqn: forge.game.ability.effects.PlayEffect
...
classDiagram
    class PlayEffect {
        ...
        +addReplaceGraveyardEffect(Card c, Card hostCard, SpellAbility sa, SpellAbility tgtSA, String zone) void
        #addIllusionaryMaskReplace(Card c, SpellAbility sa) void
    }
    PlayEffect --|> SpellAbilityEffect : extends
    PlayEffect ..> AbilityKey : uses
    PlayEffect ..> Card : uses
    PlayEffect ..> CardCollection : uses
    PlayEffect ..> CardCollectionView : uses
    PlayEffect ..> CardDb : uses
    PlayEffect ..> CardStateName : uses
    PlayEffect ..> CardZoneTable : uses
    PlayEffect ..> Cost : uses
    PlayEffect ..> CostDiscard : uses
    PlayEffect ..> CostPart : uses
    PlayEffect ..> CostReveal : uses
    PlayEffect ..> Game : uses
    PlayEffect ..> ManaCostBeingPaid : uses
    PlayEffect ..> PaperCard : uses
    PlayEffect ..> Player : uses
    PlayEffect ..> ReplacementEffect : uses
    PlayEffect ..> SpellAbility : uses
    PlayEffect ..> Zone : uses
    PlayEffect ..> ZoneType : uses
```
...
PlayEffect implements the resolution logic for "play"/"cast" spell abilities, letting one card cause other cards to be played or cast under controlled conditions. As a concrete subclass of `SpellAbilityEffect`, it overrides `getStackDescription` to render the effect's textual stack entry and `resolve` to perform the work: it gathers a candidate `CardCollection` from defined targets, a `Valid` filter over game zones, or freshly minted `PaperCard`/`CardDb` instances (for copy- or token-style effects), then drives the controlling `Player` through repeated selection and playing of those cards.

The design centers on a single large, parameter-driven `resolve` loop that honors a broad set of script parameters (alternative or modified costs, CMC limits, transformation, remember/imprint/forget bookkeeping, and temporary controller changes), reflecting Forge's data-driven card-scripting model. It collaborates with `AbilityUtils` to derive playable `SpellAbility` options, tracks zone movement via `CardZoneTable` for triggers, and installs `ReplacementEffect`s through helper methods to support corner cases like graveyard-replacement exile and the Illusionary Mask face-down mechanic.
