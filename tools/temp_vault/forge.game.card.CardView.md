...
classDiagram
    class CardView {
        ...
        -String zoneText
        +get(Card c) CardView
        +getState(Card c, CardStateName state) CardStateView
        +getStateMap(Iterable~CardState~ states) Map~CardStateView,CardState~
        +getBackup() CardView
        +getCardForUi(IPaperCard pc) CardView
        +getCollection(Iterable~Card~ cards) TrackableCollection~CardView~
        +mayViewAny(Iterable~CardView~ cards, Iterable~PlayerView~ viewers) boolean
        #updateName(GameEntity e) void
        +getOracleName() String
        +getOwner() PlayerView
        ~updateOwner(Card c) void
        +getController() PlayerView
        ~updateController(Card c) void
        +getZone() ZoneType
        ~updateZone(Card c) void
        +isInZone(Iterable~ZoneType~ zones) boolean
        +isCloned() boolean
        +isFaceDown() boolean
        +isForeTold() boolean
        +isFlipCard() boolean
        ...
        +hasSecondaryState() boolean
        +isModalCard() boolean
        +isRoom() boolean
        +getFacedownImageKey() String
        +isAttacking() boolean
        ~updateAttacking(Card c) void
        +isExertedThisTurn() boolean
        ~updateExertedThisTurn(Card c, boolean exerted) void
        +isDetained() boolean
        ~updateDetained(Card c) void
        +isBlocking() boolean
        ~updateBlocking(Card c) void
        +isPhasedOut() boolean
        ~updatePhasedOut(Card c) void
        +isFirstTurnControlled() boolean
        +hasSickness() boolean
        +isSick() boolean
        ~updateSickness(Card c) void
        +isTapped() boolean
        ~updateTapped(Card c) void
        +getGamePieceType() GamePieceType
        ~updateGamePieceType(Card c) void
        +isToken() boolean
        ~updateToken(Card c) void
        +isImmutable() boolean
        +isEmblem() boolean
        +updateEmblem(Card c) void
        +isBoon() boolean
        +updateBoon(Card c) void
        +canSpecialize() boolean
        +updateSpecialize(Card c) void
        +isTokenCard() boolean
        ~updateTokenCard(Card c) void
        +isCommander() boolean
        ~updateCommander(Card c) void
        +getCommanderType() String
        +getCounters() Map~CounterType,Integer~
        +getCounters(CounterType counterType) int
        +hasSameCounters(CardView otherCard) boolean
        +hasSamePT(CardView otherCard) boolean
        ~updateCounters(Card c) void
        +getCrackOverlayInt() int
        +getDamage() int
        ~updateDamage(Card c) void
        +getAssignedDamage() int
        ~updateAssignedDamage(Card c) void
        +getLethalDamage() int
        ~updateLethalDamage(Card c) void
        +getShieldCount() int
        ~updateShieldCount(Card c) void
        +getChosenType() String
        ~updateChosenType(Card c) void
        +getChosenType2() String
        ~updateChosenType2(Card c) void
        +getNotedTypes() List~String~
        ~updateNotedTypes(Card c) void
        +getChosenNumber() String
        ~updateChosenNumber(Card c) void
        ~clearChosenNumber() void
        +getStoredRolls() List~String~
        ~updateStoredRolls(Card c) void
        +getChosenColors() List~String~
        ~updateChosenColors(Card c) void
        +hasPaperFoil() boolean
        ~updatePaperFoil(boolean v) void
        +getMarkedColors() ColorSet
        ~updateMarkedColors(Card c) void
        +getMergedCardsCollection() FCollectionView~CardView~
        +getChosenCards() FCollectionView~CardView~
        +getChosenPlayer() PlayerView
        ~updateChosenPlayer(Card c) void
        +getPromisedGift() PlayerView
        ~updatePromisedGift(Card c) void
        +getProtectingPlayer() PlayerView
        ~updateProtectingPlayer(Card c) void
        +getChosenDirection() Direction
        ~updateChosenDirection(Card c) void
        +getChosenEvenOdd() EvenOdd
        ~updateChosenEvenOdd(Card c) void
        +getChosenMode() String
        ~updateChosenMode(Card c) void
        +getCurrentRoom() String
        ~updateCurrentRoom(Card c) void
        +getIntensity() int
        ~updateIntensity(Card c) void
        +getClassLevel() int
        ~updateClassLevel(Card c) void
        +getRingLevel() int
        ~updateRingLevel(Card c) void
        +getOverlayText() String
        +getMarkerText() List~String~
        ~updateMarkerText(Card c) void
        -getRemembered() String
        ~updateRemembered(Card c) void
        +getSector() String
        ~updateSector(Card c) void
        +getSprocket() int
        ~updateSprocket(Card c) void
        +getDraftAction() List~String~
        ~updateDraftAction(Card c) void
        +getNamedCard() List~String~
        ~updateNamedCard(Card c) void
        +getMayPlayPlayers(PlayerView pv) boolean
        ~setMayPlayPlayers(Iterable~Player~ list) void
        +mayPlayerLook(PlayerView pv) boolean
        ~setPlayerMayLook(Iterable~Player~ list) void
        +canBeShownToAny(Iterable~PlayerView~ viewers) boolean
        +canBeShownTo(PlayerView viewer) boolean
        +canFaceDownBeShownToAny(Iterable~PlayerView~ viewers) boolean
        +canFaceDownBeShownTo(PlayerView viewer) boolean
        +getEncodedCards() FCollectionView~CardView~
        +getUntilLeavesBattlefield() FCollectionView~CardView~
        +getEntityAttachedTo() GameEntityView
        ~updateAttachedTo(Card c) void
        +getAttachedTo() CardView
        +getEnchantedPlayer() PlayerView
        +getGainControlTargets() FCollectionView~CardView~
        +getCloneOrigin() CardView
        +getExiledWith() CardView
        +getImprintedCards() FCollectionView~CardView~
        +getExiledCards() FCollectionView~CardView~
        +getHauntedBy() FCollectionView~CardView~
        +getHaunting() CardView
        +getMustBlockCards() FCollectionView~CardView~
        ~updateMustBlockCards(Card c) void
        +getPairedWith() CardView
        +getChangedColorWords() Map~String,String~
        ~updateChangedColorWords(Card c) void
        +getChangedTypes() Map~String,String~
        ~updateChangedTypes(Card c) void
        ~updateNonAbilityText(Card c) void
        +getText() String
        +getText(CardStateView state, HashMap~String,String~ translationsText) String
        +getCurrentState() CardStateView
        +hasAlternateState() boolean
        +getAlternateState() CardStateView
        +hasLeftSplitState() boolean
        +getLeftSplitState() CardStateView
        +hasRightSplitState() boolean
        +getRightSplitState() CardStateView
        +hasBackSide() boolean
        +getBackSideName() String
        +createAlternateState(CardStateName state0) CardStateView
        +getState(boolean alternate0) CardStateView
        ~updateBackSide(String stateName, boolean hasBackSide) void
        +wasDestroyed() boolean
        ~updateWasDestroyed(boolean value) void
        +needsUntapAnimation() boolean
        +updateNeedsUntapAnimation(boolean value) void
        +needsTapAnimation() boolean
        +updateNeedsTapAnimation(boolean value) void
        +needsTransformAnimation() boolean
        +updateNeedsTransformAnimation(boolean value) void
        ~updateState(Card c) void
        +getHiddenId() int
        ~updateHiddenId(int hiddenId) void
        ~getBlockAdditional() int
        ~getCanBlockAny() boolean
        ~updateBlockAdditional(Card c) void
        +isRingBearer() boolean
        ~updateRingBearer(Card c) void
        ~getCantHaveKeyword() Set~String~
        ~updateCantHaveKeyword(Card c) void
        -getZoneText() String
        ~updateZoneText(Card c) void
        +toString() String
        ~setCard(Card oldCard, Card newCard, TrackableProperty key) Card
        ~setCards(CardCollection oldCards, CardCollection newCards, TrackableProperty key) CardCollection
        ~setCards(CardCollection oldCards, Iterable~Card~ newCards, TrackableProperty key) CardCollection
        ~addCard(CardCollection oldCards, Card cardToAdd, TrackableProperty key) CardCollection
        ~addCards(CardCollection oldCards, Iterable~Card~ cardsToAdd, TrackableProperty key) CardCollection
        ~removeCard(CardCollection oldCards, Card cardToRemove, TrackableProperty key) CardCollection
        ~removeCards(CardCollection oldCards, Iterable~Card~ cardsToRemove, TrackableProperty key) CardCollection
        ~clearCards(CardCollection oldCards, TrackableProperty key) CardCollection
        ~updateMergeCollections(CardCollection cards) void
        +CardView(int id0, Tracker tracker)
        +CardView(int id0, Tracker tracker, String name0)
        +CardView(int id0, Tracker tracker, String name0, PlayerView ownerAndController, String imageKey)
    }
    CardView --|> GameEntityView : extends
    CardView ..> AbilityManaPart : uses
    CardView ..> Card : uses
    CardView ..> CardCollection : uses
    CardView ..> CardCollectionView : uses
    CardView ..> CardRarity : uses
    CardView ..> CardRules : uses
    CardView ..> CardState : uses
    CardView ..> CardStateName : uses
    CardView ..> CardStateView : uses
    CardView ..> CardTypeView : uses
    CardView ..> ColorSet : uses
    CardView ..> Combat : uses
    CardView ..> CounterType : uses
    CardView ..> Direction : uses
    CardView ..> EvenOdd : uses
    CardView ..> FCollectionView : uses
    CardView ..> GameEntity : uses
    CardView ..> GamePieceType : uses
    CardView ..> IPaperCard : uses
    CardView ..> ITranslatable : uses
    CardView ..> Keyword : uses
    CardView ..> KeywordCollectionView : uses
    CardView ..> ManaCost : uses
    CardView ..> Player : uses
    CardView ..> PlayerView : uses
    CardView ..> SpellAbility : uses
    CardView ..> TrackableCollection : uses
    CardView ..> TrackableObject : uses
    CardView ..> TrackableProperty : uses
    CardView ..> Tracker : uses
    CardView ..> ZoneType : uses
```
...
The CardView class is a read-only, serializable view of a game Card, used to present card information to the UI and to remote (networked) clients without exposing or directly coupling to the mutable game-state Card object. Extending GameEntityView, it stores all displayable attributes â€” name, owner, controller, zone, counters, damage, combat status, chosen values, attachments, and related card collections â€” as TrackableProperty entries, so that changes are tracked and synchronized through the Tracker framework rather than read live from the Card.

Its many `update*` methods (package-private) snapshot state from a source Card, while public getters serve the UI; static factories like `get`, `getCollection`, and `getCardForUi` build views from cards. A key design responsibility is information hiding: `canBeShownTo`/`mayViewAny` enforce per-zone, per-viewer visibility rules (hidden hands, face-down cards, mind-slave control) so players only see what the rules permit. The inner CardStateView models each card face/state (power, toughness, colors, types, keywords, mana), supporting split, double-faced, flip, and merged cards.
...
## Python
`forge/game/card/CardView.py`

```python
if alternateState is None:
            self.set(TrackableProperty.AlternateState, None)
        else:
            alternateStateView = alternateState.getView()
            if self.getAlternateState() is not alternateStateView:
                self.set(TrackableProperty.AlternateState, alternateStateView)
                alternateStateView.updateName(alternateState)
                alternateStateView.updatePower(c)  # ensure power, toughness, and loyalty updated when current state changes
                alternateStateView.updateToughness(c)
                alternateStateView.updateLoyalty(c)
    ...
    def getHiddenId(self):
        hiddenId = super().get(TrackableProperty.HiddenId)
        if hiddenId is None:
            return self.getId()
        return hiddenId

    def updateHiddenId(self, hiddenId):
        self.set(TrackableProperty.HiddenId, hiddenId)

    def getBlockAdditional(self):
        return super().get(TrackableProperty.BlockAdditional)

    def getCanBlockAny(self):
        return super().get(TrackableProperty.BlockAny)

    def updateBlockAdditional(self, c):
        self.set(TrackableProperty.BlockAdditional, c.canBlockAdditional())
        self.set(TrackableProperty.BlockAny, c.canBlockAny())

    def isRingBearer(self):
        return super().get(TrackableProperty.IsRingBearer)

    def updateRingBearer(self, c):
        self.set(TrackableProperty.IsRingBearer, c.isRingBearer())

    def getCantHaveKeyword(self):
        return super().get(TrackableProperty.CantHaveKeyword)

    def updateCantHaveKeyword(self, c):
        keywords = set()
        for k in c.getCantHaveKeyword():
            keywords.add(str(k))
        self.set(TrackableProperty.CantHaveKeyword, keywords)

    def getZoneText(self):
        return self.zoneText

    def updateZoneText(self, c):
        if c.getZone() is not None and c.getZone().is_(ZoneType.Sideboard) and c.getGame().getMaingame() is not None:
            parentCard = c.getOwner().getMappingMaingameCard(c)
            sb = ""
            parentLevel = 0
            while parentCard is not None:
                parentLevel += 1
                if not parentCard.getZone().is_(ZoneType.Sideboard):
                    sb += '['
                    if parentCard.getGame().getMaingame() is None:
                        sb += Localizer.getInstance().getMessage("lblMainGame")
                    else:
                        sb += Localizer.getInstance().getMessage("lblSubgame", parentLevel)
                    sb += ": "
                    sb += TextUtil.capitalize(parentCard.getZone().getZoneType().getTranslatedName())
                    sb += ']'
                    break
                parentCard = parentCard.getOwner().getMappingMaingameCard(parentCard)
            self.zoneText = sb
    ...
    def toString(self):
        name = self.getName()
        zone = self.getZoneText()
        if self.getId() <= 0:  # if fake card, just return name
            return name

        if name == "":
            alternate = self.getAlternateState()
            if alternate is not None:
                if self.isFaceDown():
                    return "Face-down card (H" + str(self.getHiddenId()) + ")"
                else:
                    return self.getAlternateState().getName() + " (" + str(self.getId()) + ")"
        return (zone + ' ' + CardTranslation.getTranslatedName(name) + " (" + str(self.getId()) + ")").strip()
    ...
    class CardStateView(TrackableObject, ITranslatable):
        ...
        def __init__(self, id0, state0, tracker):
        ...
        def getDisplayId(self):
            if self.getState() == CardStateName.FaceDown:
                return "H" + str(self._outer.getHiddenId())
            id = self.getId()
            if id > 0:
        ...
        def updateName(self, c):
            card = c.getCard()
            self.setName(card.getDisplayName(c))
            self.setOracleName(card.getName(c))

            if self._outer.getCurrentState() is self:
                self._outer.updateName(card)
        ...
        def updateColors(self, c):
            self.set(TrackableProperty.Colors, c.getColor())

        def setOriginalColors(self, c):
            self.set(TrackableProperty.OriginalColors, c.getColor())

        def updateHasChangeColors(self, hasChangeColor):
            self.set(TrackableProperty.HasChangedColors, hasChangeColor)

        def hasChangeColors(self):
            return self.get(TrackableProperty.HasChangedColors)

        def getImageKey(self, viewers=None):
            if self.getState() == CardStateName.FaceDown:
                return self.getCard().getFacedownImageKey()
            if self._outer.canBeShownToAny(viewers):
                if self._outer.isCloned() and StaticData.instance().useSourceImageForClone():
                    return self._outer.getBackup().getCurrentState().getImageKey(viewers)
                return self.get(TrackableProperty.ImageKey)
            return ImageKeys.getTokenKey(ImageKeys.HIDDEN_CARD)

        # Use this for revealing purposes only
        def getTrackableImageKey(self):
            return self.get(TrackableProperty.ImageKey)

        def updateImageKey(self, c):
            self.set(TrackableProperty.ImageKey, c.getImageKey())

        def getType(self):
            if self.getState() != CardStateName.Original and self._outer.isFaceDown() and not self._outer.isInZone({ZoneType.Battlefield, ZoneType.Stack}):
                return CardType.EMPTY
            return self.get(TrackableProperty.Type)

        def updateType(self, c):
            self.set(TrackableProperty.Type, c.getTypeWithChanges())

        def getManaCost(self):
            return self.get(TrackableProperty.ManaCost)

        def getOriginalManaCost(self):
            return self.get(TrackableProperty.OriginalManaCost)

        def updateManaCost(self, c):
        ...
        def setFunctionalVariantName(self, functionalVariant):
        ...
        def updateRulesText(self, rules):
            rulesText = None
        ...
        def updatePower(self, c):
            if isinstance(c, Card):
                if self.hasPrintedPT() and not self.isCreature():
                    ...
                    num = c.getNetPower()
                if c.getCurrentState().getView() is not self and c.getAlternateState() is not None:
                    num = num - c.getBasePower() + c.getAlternateState().getBasePower()
        ...
        def updateAttractionLights(self, c):
        ...
        def hasAftermath(self):
            return self.hasKeyword(Keyword.AFTERMATH)

        def hasDivideDamage(self):
            return self.get(TrackableProperty.HasDivideDamage)

        def origProduceAnyMana(self):
            return self.get(TrackableProperty.OrigProduceAnyMana)

        def origProduceMana(self):
            return self.get(TrackableProperty.OrigProduceMana)

        def getAbilityText(self):
            return self.get(TrackableProperty.AbilityText)

        def updateAbilityText(self, c, state):
            self.set(TrackableProperty.AbilityText, c.getAbilityText(state))

        def updateKeywords(self, c, state):
            c.updateKeywordsCache(state)
            self.set(TrackableProperty.Keywords, state.getCachedKeywords().getView())
            # deeper check for Idris
            self.set(TrackableProperty.HasAnnihilator, c.hasKeyword(Keyword.ANNIHILATOR, state) or state.getTriggers().anyMatch(lambda t: t.isKeyword(Keyword.ANNIHILATOR)))
            self.set(TrackableProperty.HasWard, c.hasKeyword(Keyword.WARD, state) or state.getTriggers().anyMatch(lambda t: t.isKeyword(Keyword.WARD)))
        ...
        def updateManaColorBG(self, state):
            anyMana = False
            cMana = False
            colors = 0
    ...
    # special methods for updating card and player properties as needed and returning the new collection
    def setCard(self, oldCard, newCard, key):
        if newCard is not oldCard:
            self.set(key, CardView.get(newCard))
        return newCard

    def setCards(self, oldCards, newCards, key):
        if newCards is not None and not isinstance(newCards, CardCollection):
            if newCards is None:
                self.set(key, None)
                return None
            return self.setCards(oldCards, CardCollection(newCards), key)
        if newCards is None or newCards.isEmpty():  # avoid storing empty collections
            self.set(key, None)
            return None
        self.set(key, CardView.getCollection(newCards))  # TODO prevent overwriting list if not necessary
        return newCards

    def addCard(self, oldCards, cardToAdd, key):
        if cardToAdd is None:
            return oldCards

        if oldCards is None:
            oldCards = CardCollection()
        if oldCards.add(cardToAdd):
            views = super().get(key)
            if views is None:
                views = TrackableCollection()
                views.add(cardToAdd.getView())
                self.set(key, views)
            elif views.add(cardToAdd.getView()):
                self.flagAsChanged(key)
        return oldCards

    def addCards(self, oldCards, cardsToAdd, key):
        if cardsToAdd is None:
            return oldCards
        ...
        for c in cardsToAdd:
            if c is not None and oldCards.add(c):
                if views is None:
                    views = TrackableCollection()
                    views.add(c.getView())
                    self.set(key, views)
                elif views.add(c.getView()):
                    needFlagAsChanged = True
        if needFlagAsChanged:
            self.flagAsChanged(key)
        return oldCards

    def removeCard(self, oldCards, cardToRemove, key):
        if cardToRemove is None or oldCards is None:
            return oldCards
        ...
        if oldCards.remove(cardToRemove):
            ...
            if views is None:
                self.set(key, None)
            elif views.remove(cardToRemove.getView()):
                if views.isEmpty():
                    self.set(key, None)  # avoid keeping around an empty collection
                else:
                    self.flagAsChanged(key)
            if oldCards.isEmpty():
                oldCards = None  # avoid keeping around an empty collection
        return oldCards

    def removeCards(self, oldCards, cardsToRemove, key):
        if cardsToRemove is None or oldCards is None:
            return oldCards
    ...
    def clearCards(self, oldCards, key):
        if oldCards is not None:
            self.set(key, None)
        return None

    def updateMergeCollections(self, cards):
        views = super().get(TrackableProperty.MergedCardsCollection)
        needFlagAsChanged = False
        if views is None:
            views = TrackableCollection()
            self.set(TrackableProperty.MergedCardsCollection, views)
        else:
        ...
        if cards is not None:
            for c in cards:
                if views.add(c.getView()):
                    needFlagAsChanged = True
        if needFlagAsChanged:
            self.flagAsChanged(TrackableProperty.MergedCardsCollection)
```
