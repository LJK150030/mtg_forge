...
AiProps is a Java enumeration that catalogs the complete set of tunable parameters governing the Forge AI's decision-making, pairing each setting identifier with a string-typed default value. Each constant—spanning domains such as mulligan thresholds, combat trading, counterspell odds, scrying, flash timing, and sideboarding—is built through the package-private constructor with its default, retrievable via `getDefault()`. Storing every default as a single `String` field keeps the enum agnostic to the underlying data type (boolean, integer, percentage, or named strategy), deliberately deferring parsing to consumers.
