...
classDiagram
    class AiProps {
        <<enumeration>>
        DEFAULT_MAX_PLANAR_DIE_ROLLS_PER_TURN
        DEFAULT_MIN_TURN_TO_ROLL_PLANAR_DIE
        DEFAULT_PLANAR_DIE_ROLL_CHANCE
        MULLIGAN_THRESHOLD
        PLANAR_DIE_ROLL_HESITATION_CHANCE
        HOLD_LAND_DROP_FOR_MAIN2_IF_UNUSED
        HOLD_LAND_DROP_ONLY_IF_HAVE_OTHER_PERMS
        CHEAT_WITH_MANA_ON_SHUFFLE
        MOVE_EQUIPMENT_TO_BETTER_CREATURES
        MOVE_EQUIPMENT_CREATURE_EVAL_THRESHOLD
        PRIORITIZE_MOVE_EQUIPMENT_IF_USELESS
        SAC_TO_REATTACH_TARGET_EVAL_THRESHOLD
        PREDICT_SPELLS_FOR_MAIN2
        RESERVE_MANA_FOR_MAIN2_CHANCE
        PLAY_AGGRO
        ...
        TRY_TO_AVOID_ATTACKING_INTO_CERTAIN_BLOCK
        TRY_TO_HOLD_COMBAT_TRICKS_UNTIL_BLOCK
        CHANCE_TO_HOLD_COMBAT_TRICKS_UNTIL_BLOCK
        ...
        MIN_CHANCE_TO_RANDOMLY_TRADE_ON_BLOCK
        MAX_CHANCE_TO_RANDOMLY_TRADE_ON_BLOCK
        CHANCE_DECREASE_TO_TRADE_VS_EMBALM
        CHANCE_TO_TRADE_TO_SAVE_PLANESWALKER
        CHANCE_TO_TRADE_DOWN_TO_SAVE_PLANESWALKER
        THRESHOLD_TOKEN_CHUMP_TO_SAVE_PLANESWALKER
        THRESHOLD_NONTOKEN_CHUMP_TO_SAVE_PLANESWALKER
        CHUMP_TO_SAVE_PLANESWALKER_ONLY_ON_LETHAL
        TRY_TO_PRESERVE_BUYBACK_SPELLS
        MIN_SPELL_CMC_TO_COUNTER
        CHANCE_TO_COUNTER_CMC_1
        CHANCE_TO_COUNTER_CMC_2
        CHANCE_TO_COUNTER_CMC_3
        ALWAYS_COUNTER_OTHER_COUNTERSPELLS
        ALWAYS_COUNTER_DAMAGE_SPELLS
        ALWAYS_COUNTER_CMC_0_MANA_MAKING_PERMS
        ALWAYS_COUNTER_REMOVAL_SPELLS
        ALWAYS_COUNTER_PUMP_SPELLS
        ALWAYS_COUNTER_AURAS
        ALWAYS_COUNTER_SPELLS_FROM_NAMED_CARDS
        CHANCE_TO_COPY_OWN_SPELL_WHILE_ON_STACK
        ALWAYS_COPY_SPELL_IF_CMC_DIFF
        ACTIVELY_DESTROY_ARTS_AND_NONAURA_ENCHS
        ACTIVELY_DESTROY_IMMEDIATELY_UNBLOCKABLE
        ACTIVELY_PROTECT_VS_CURSE_AURAS
        ...
        AVOID_TARGETING_CREATS_THAT_WILL_DIE
        DONT_EVAL_KILLSPELLS_ON_STACK_WITH_PERMISSION
        CHANCE_TO_CHAIN_TWO_DAMAGE_SPELLS
        HOLD_X_DAMAGE_SPELLS_FOR_MORE_DAMAGE_CHANCE
        HOLD_X_DAMAGE_SPELLS_THRESHOLD
        PRIORITY_REDUCTION_FOR_STORM_SPELLS
        USE_BERSERK_AGGRESSIVELY
        MIN_COUNT_FOR_STORM_SPELLS
        STRIPMINE_MIN_LANDS_IN_HAND_TO_ACTIVATE
        STRIPMINE_MIN_LANDS_FOR_NO_TIMING_CHECK
        STRIPMINE_MIN_LANDS_OTB_FOR_NO_TEMPO_CHECK
        STRIPMINE_MAX_LANDS_TO_ATTEMPT_MANALOCKING
        STRIPMINE_HIGH_PRIORITY_ON_SKIPPED_LANDDROP
        TOKEN_GENERATION_ABILITY_CHANCE
        TOKEN_GENERATION_ALWAYS_IF_FROM_PLANESWALKER
        TOKEN_GENERATION_ALWAYS_IF_OPP_ATTACKS
        SCRY_NUM_LANDS_TO_STILL_NEED_MORE
        SCRY_NUM_LANDS_TO_NOT_NEED_MORE
        SCRY_NUM_CREATURES_TO_NOT_NEED_SUBPAR_ONES
        SCRY_EVALTHR_CREATCOUNT_TO_SCRY_AWAY_LOWCMC
        SCRY_EVALTHR_TO_SCRY_AWAY_LOWCMC_CREATURE
        SCRY_EVALTHR_CMC_THRESHOLD
        SCRY_IMMEDIATELY_UNCASTABLE_TO_BOTTOM
        SCRY_IMMEDIATELY_UNCASTABLE_CMC_DIFF
        SURVEIL_NUM_CARDS_IN_LIBRARY_TO_BAIL
        SURVEIL_LIFEPERC_AFTER_PAYING_LIFE
        COMBAT_ASSAULT_ATTACK_EVASION_PREDICTION
        COMBAT_ATTRITION_ATTACK_EVASION_PREDICTION
        CONSERVATIVE_ENERGY_PAYMENT_ONLY_IN_COMBAT
        CONSERVATIVE_ENERGY_PAYMENT_ONLY_DEFENSIVELY
        BOUNCE_ALL_TO_HAND_CREAT_EVAL_DIFF
        BOUNCE_ALL_ELSEWHERE_CREAT_EVAL_DIFF
        BOUNCE_ALL_TO_HAND_NONCREAT_EVAL_DIFF
        BOUNCE_ALL_ELSEWHERE_NONCREAT_EVAL_DIFF
        INTUITION_ALTERNATIVE_LOGIC
        EXPLORE_MAX_CMC_DIFF_TO_PUT_IN_GRAVEYARD
        EXPLORE_NUM_LANDS_TO_STILL_NEED_MORE
        MOMIR_BASIC_LAND_STRATEGY
        MOJHOSTO_NUM_LANDS_TO_ACTIVATE_JHOIRA
        MOJHOSTO_CHANCE_TO_PREFER_JHOIRA_OVER_MOMIR
        MOJHOSTO_CHANCE_TO_USE_JHOIRA_COPY_INSTANT
        AI_IN_DANGER_THRESHOLD
        AI_IN_DANGER_MAX_THRESHOLD
        ...
        FLASH_BUFF_AURA_CHANCE_TO_CAST_EARLY
        FLASH_BUFF_AURA_CHANCE_CAST_AT_EOT
        FLASH_BUFF_AURA_CHANCE_TO_RESPOND_TO_STACK
        BLINK_RELOAD_PLANESWALKER_CHANCE
        BLINK_RELOAD_PLANESWALKER_MAX_LOYALTY
        BLINK_RELOAD_PLANESWALKER_LOYALTY_DIFF
        SACRIFICE_DEFAULT_PREF_ENABLE
        SACRIFICE_DEFAULT_PREF_MIN_CMC
        SACRIFICE_DEFAULT_PREF_MAX_CMC
        SACRIFICE_DEFAULT_PREF_ALLOW_TOKENS
        SACRIFICE_DEFAULT_PREF_MAX_CREATURE_EVAL
        SIDEBOARDING_ENABLE
        SIDEBOARDING_CHANCE_PER_CARD
        SIDEBOARDING_CHANCE_ON_WIN
        SIDEBOARDING_IN_LIMITED_FORMATS
        SIDEBOARDING_SHARED_TYPE_ONLY
        SIDEBOARDING_PLANESWALKER_EQ_CREATURE
...
AiProps is a Java enumeration that catalogs the complete set of tunable parameters governing the Forge AI's decision-making, pairing each setting identifier with a string-typed default value. Each constantâ€”spanning domains such as mulligan thresholds, combat trading, counterspell odds, scrying, flash timing, and sideboardingâ€”is built through the package-private constructor with its default, retrievable via `getDefault()`. Storing every default as a single `String` field keeps the enum agnostic to the underlying data type (boolean, integer, percentage, or named strategy), deliberately deferring parsing to consumers.

As its Javadoc states, the enum acts as the schema of keys for an AI personality profile: at runtime its values seed a map that is overlaid with profile settings loaded from a text file, letting AI behavior be configured externally while these constants supply the authoritative key set and fallback defaults.
...
class AiProps(Enum):
    def __new__(cls, s0: str):
        obj = object.__new__(cls)
        obj._value_ = len(cls.__members__)
        obj.strDefaultVal = s0
        return obj
