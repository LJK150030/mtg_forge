...
fqn: forge.game.ability.effects.FightEffect
...
classDiagram
    class FightEffect {
        ...
        -getFighters(SpellAbility sa) List~Card~
        -dealDamage(SpellAbility sa, Card fighterA, Card fighterB) void
    }
    FightEffect --|> DamageBaseEffect : extends
    FightEffect ..> AbilityKey : uses
    FightEffect ..> Card : uses
    FightEffect ..> CardCollection : uses
    FightEffect ..> CardCollectionView : uses
    FightEffect ..> CardDamageMap : uses
    FightEffect ..> Game : uses
    FightEffect ..> GameEntityCounterTable : uses
    FightEffect ..> Player : uses
    FightEffect ..> SpellAbility : uses
```
...
FightEffect resolves Magic's "fight" abilities, in which two creatures simultaneously deal damage to each other equal to their power. It extends DamageBaseEffect to inherit shared damage scaffolding (notably the replaceDying handling it invokes) and, as a concrete SpellAbilityEffect, overrides getStackDescription for rendering and resolve for execution, delegating combatant selection to getFighters and damage application to dealDamage.

The class encodes the comprehensive rules directly: getFighters revalidates each candidate against its game-timestamped CardState to ensure it is still a legal in-play creature (701.12b), and dealDamage doubles power when a creature fights itself (701.12c). It routes simultaneous damage through a shared CardDamageMap, preventMap, and GameEntityCounterTable, gates optional fights on the controlling Player's confirmation, and fires Fight and FightOnce triggers keyed by AbilityKey through the Game's trigger handler.

## Source
`forge-game/src/main/java/forge/game/ability/effects/FightEffect.java`
