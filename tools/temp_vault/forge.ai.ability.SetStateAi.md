...
fqn: forge.ai.ability.SetStateAi
...
classDiagram
    class SetStateAi {
        ...
        -shouldTransformCard(Card card, Player ai, PhaseHandler ph) boolean
        -shouldTurnFace(Card card, Player ai, PhaseHandler ph, String mode) boolean
        -compareCards(Card original, Card copy, Player ai, PhaseHandler ph) boolean
        -isSafeToTransformIntoLegendary(Player aiPlayer, Card source) boolean
        +confirmAction(Player player, SpellAbility sa, PlayerActionConfirmMode mode, String message, Map~String,Object~ params) boolean
    }
    SetStateAi --|> SpellAbilityAi : extends
    SetStateAi ..> AiAbilityDecision : uses
    SetStateAi ..> Card : uses
    SetStateAi ..> CardCollection : uses
    SetStateAi ..> CardState : uses
    SetStateAi ..> CounterType : uses
    SetStateAi ..> PhaseHandler : uses
    SetStateAi ..> Player : uses
    SetStateAi ..> PlayerActionConfirmMode : uses
    SetStateAi ..> SpellAbility : uses
```
