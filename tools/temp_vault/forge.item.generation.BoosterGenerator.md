...
fqn: forge.item.generation.BoosterGenerator
...
classDiagram
    class BoosterGenerator {
        -Map~String,String~ staticSheetsCorrespondance
        -Map~String,PrintSheet~ cachedSheets
        -getPrintSheet(String key) PrintSheet
        -generateFoilCard(PrintSheet sheet) PaperCard
        -generateFoilCard(List~PaperCard~ cardList) PaperCard
        +tryGetStaticSheet(String sheetName) PrintSheet
        +getBoosterPack(SealedTemplate template) List~PaperCard~
        +getBoosterPack(SealedTemplateWithSlots template) List~PaperCard~
        -bulkSlotReplacement(BoosterSlot boosterSlot, int numCards) Map~String,Long~
        -ensureGuaranteedCardInBooster(List~PaperCard~ result, SealedTemplate template, String boosterMustContain) void
        +replaceCardFromExtraSheet(List~PaperCard~ booster, String printSheetKey) void
        +replaceCard(List~PaperCard~ booster, PaperCard toAdd) void
        +addCardsFromExtraSheet(List~PaperCard~ dest, String printSheetKey) void
        +makeSheet(String sheetKey, Iterable~PaperCard~ src) PrintSheet
        -buildExtraPredicate(List~String~ operators) Predicate~PaperCard~
    }
    BoosterGenerator ..> BoosterSlot : uses
    BoosterGenerator ..> CardEdition : uses
    BoosterGenerator ..> CardRarity : uses
    BoosterGenerator ..> PaperCard : uses
    BoosterGenerator ..> PrintSheet : uses
    BoosterGenerator ..> SealedTemplate : uses
    BoosterGenerator ..> SealedTemplateWithSlots : uses
```
