...
classDiagram
    class NameGenerator {
        -String[] fantasyMales
        -String[] fantasyFemales
        -String[] genericMales
        -String[] genericFemales
        -String[] monikers
        -List~String~ usedMonikers
        -List~String~ usedNames
        -String[] sourceList
        +getRandomName(String gender, String type, List~String~ excludeNames) String
        +getRandomNames(int generateAmount, List~String~ excludeNames) List~String~
        +getRandomName(String gender, String type, String notNamed) String
        -getCombinedLists(String[] listOne, String[] listTwo) String[]
    }
```

## Design Description

The NameGenerator class is a stateless utility for procedurally generating character names within Forge's `forge.util` package. Declared `final` with only static members and static data arrays of fantasy and generic male/female given names plus decorative monikers, it exposes overloaded `getRandomName` methods and a batch `getRandomNames` method that produce randomized names filtered against a caller-supplied exclusion list. A gender/type key selects the appropriate source arrayâ€”optionally combining lists via the private `getCombinedLists` helperâ€”and applies a small, category-tuned probability of appending a unique moniker.

As a leaf utility, it collaborates with `MyRandom` for its randomness source and Apache Commons `ArrayUtils` for array concatenation, while tracking already-used names and monikers in static lists to discourage repeats. The design favors lightweight static access over instantiation, trading thread-safety and shared mutable state for simplicity suited to one-off name generation.

## Source
`forge-core/src/main/java/forge/util/NameGenerator.java`
...
class NameGenerator:
    ...
    usedMonikers: list[str] = []
    usedNames: list[str] = None
    sourceList: list[str] = None

    @staticmethod
    def getRandomName(gender: str, type: str, excludeNames):
        """Generates a single name that doesn't match the specified name(s).

        gender        String specifying the desired gender. Recognizes "Male", "Female" and "Any".
        type          String specifying the desired type. Recognizes "Fantasy", "Generic" and "Any".
        excludeNames  A list of names already being used, or a single name string.
        """
        # Overload: getRandomName(gender, type, notNamed: String)
        if isinstance(excludeNames, str):
            notNamed = excludeNames
            exclude = []
            exclude.append(notNamed)
            return NameGenerator.getRandomName(gender, type, exclude)

        NameGenerator.usedNames = excludeNames
        name = ""
        ...
        key = type + gender
        if key == "GenericMale":
            NameGenerator.sourceList = NameGenerator.genericMales
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.03
        elif key == "GenericFemale":
            NameGenerator.sourceList = NameGenerator.genericFemales
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.02
        elif key == "FantasyMale":
            NameGenerator.sourceList = NameGenerator.fantasyMales
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.10
        elif key == "FantasyFemale":
            NameGenerator.sourceList = NameGenerator.fantasyFemales
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.08
        elif key == "AnyMale":
            NameGenerator.sourceList = NameGenerator.getCombinedLists(NameGenerator.genericMales, NameGenerator.fantasyMales)
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.06
        elif key == "AnyFemale":
            NameGenerator.sourceList = NameGenerator.getCombinedLists(NameGenerator.genericFemales, NameGenerator.fantasyFemales)
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.025
        elif key == "GenericAny":
            NameGenerator.sourceList = NameGenerator.getCombinedLists(NameGenerator.genericMales, NameGenerator.genericFemales)
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.015
        elif key == "FantasyAny":
            NameGenerator.sourceList = NameGenerator.getCombinedLists(NameGenerator.fantasyMales, NameGenerator.fantasyFemales)
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.06
        else:
            all = []
            all.extend(NameGenerator.genericMales)
            all.extend(NameGenerator.fantasyMales)
            all.extend(NameGenerator.genericFemales)
            all.extend(NameGenerator.fantasyFemales)
            NameGenerator.sourceList = list(all)
            useMoniker = MyRandom.getRandom().nextFloat() <= 0.04

        while True:
            name = NameGenerator.sourceList[MyRandom.getRandom().nextInt(len(NameGenerator.sourceList))]
            if not (name in excludeNames):
                break

        NameGenerator.usedNames.append(name)  # add base name to used names list

        if useMoniker:
            moniker = ""
            while True:
                moniker = NameGenerator.monikers[MyRandom.getRandom().nextInt(len(NameGenerator.monikers))]
                if not (moniker in NameGenerator.usedMonikers):
                    break

            NameGenerator.usedMonikers.append(moniker)
            name += " " + moniker

        return name

    @staticmethod
    def getRandomNames(generateAmount: int, excludeNames: list[str]) -> list[str]:
        """Generates a specified number of random names."""
        NameGenerator.usedNames = excludeNames
        names: list[str] = []
        for i in range(generateAmount):
            NameGenerator.getRandomName("Any", "Any", NameGenerator.usedNames)
        return names

    @staticmethod
    def getCombinedLists(listOne: list[str], listTwo: list[str]) -> list[str]:
        joined = listOne + listTwo
        return joined
```
