...
## Python
`forge/card/DraftOptions/DoublePick.py`

```python
from enum import Enum, auto


class DoublePick(Enum):
    NEVER = auto()
    FIRST_PICK = auto()  # only first pick each pack
    WHEN_POD_SIZE_IS_4 = auto()  # only when pod size is 4, so you can pick two cards each time
    ALWAYS = auto()  # each time you receive a pack, you can pick two cards
```
