...
tags:
  ...
  - module/forge-ai
  - pkg/forge/ai
fqn: forge.ai.CreatureEvaluator
package: forge.ai
module: forge-ai
kind: Class
---
...
classDiagram
    class CreatureEvaluator {
        +apply(Card c) Integer
        +evaluateCreature(Card c) int
        +evaluateCreature(Card c, boolean considerPT, boolean considerCMC) int
        -evaluateSpellAbility(SpellAbility sa) int
        #addValue(int value, String text) int
        #subValue(int value, String text) int
    }
    CreatureEvaluator ..|> Function : implements
    CreatureEvaluator ..> Card : uses
    CreatureEvaluator ..> CostPayEnergy : uses
    CreatureEvaluator ..> GameEntity : uses
    CreatureEvaluator ..> SpellAbility : uses
    CreatureEvaluator ..> Trigger : uses
```
