...
classDiagram
    class ForgeScript {
        +cardStateHasProperty(CardState cardState, String property, Player sourceController, Card source, CardTraitBase spellAbility) boolean
        +spellAbilityHasProperty(SpellAbility sa, String property, Player sourceController, Card source, CardTraitBase spellAbility) boolean
    }
    ForgeScript ..> Card : uses
    ForgeScript ..> CardState : uses
    ForgeScript ..> CardTraitBase : uses
    ForgeScript ..> CardTypeView : uses
    ForgeScript ..> ColorSet : uses
    ForgeScript ..> Cost : uses
    ForgeScript ..> Game : uses
    ForgeScript ..> GameObject : uses
    ForgeScript ..> Mana : uses
    ForgeScript ..> ManaCostBeingPaid : uses
    ForgeScript ..> Player : uses
    ForgeScript ..> SpellAbility : uses
    ForgeScript ..> StaticAbility : uses
    ForgeScript ..> TargetChoices : uses
    ForgeScript ..> Trigger : uses
```
...
`ForgeScript` is a stateless utility class that centralizes the evaluation of textual property predicates used throughout Forge's card-scripting language. Its two static methods, `cardStateHasProperty` and `spellAbilityHasProperty`, each take a target object, a property string, and the evaluating context (source controller, source card, and the originating `CardTraitBase`), then dispatch on the property string to return a boolean. The card-state method resolves color, type, mana-cost, chosen-value, and ability predicates, while the spell-ability method resolves alternative-cost keywords, targeting, mana-spent, and timing predicates.

As a pure dispatcher it holds no state and instantiates nothing, instead collaborating widely: it queries `CardState`, `Card`, `SpellAbility`, and their `CardTypeView`, `ColorSet`, `Cost`, and `Mana` data, and delegates back to `isValid`/`hasProperty` for recursive checks and to `StaticAbility` helpers and `AbilityUtils`/`Expressions` for static-ability and numeric-comparison logic. The long `if/else` chains and string-parsing reflect a deliberate design where Forge's external card scripts drive behavior through declarative property names rather than compiled code, with unmatched properties falling through to type-name matching.
