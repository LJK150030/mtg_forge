...
classDiagram
    class AiController {
        ...
        -Game game
        -AiCardMemory memory
        -Combat predictedCombat
        -Combat predictedCombatNextTurn
        -boolean useSimulation
        -SpellAbilityPicker simPicker
        -int lastAttackAggression
        -boolean useLivingEnd
        -List~SpellAbility~ skipped
        -boolean timeoutReached
        +usesSimulation() boolean
        +setUseSimulation(boolean value) void
        +getAttackAggression() int
        +getSimulationPicker() SpellAbilityPicker
        +getGame() Game
        +getPlayer() Player
        +getCardMemory() AiCardMemory
        +getPredictedCombat() Combat
        +getPredictedCombatNextTurn() Combat
        -getPossibleETBCounters() List~SpellAbility~
        -checkCurseEffects(SpellAbility sa) boolean
        +checkETBEffects(Card card, SpellAbility sa, ApiType api) boolean
        -checkETBEffectsPreparedCard(Card card, SpellAbility sa, ApiType api) boolean
        -getPlayableCounters(CardCollection all) List~SpellAbility~
        -filterLandsToPlay(CardCollection landList) CardCollection
        -chooseBestLandToPlay(CardCollection landList) Card
        -chooseCounterSpell(List~SpellAbility~ possibleCounters) SpellAbility
        +predictSpellToCastInMain2(ApiType exceptSA) SpellAbility
        -predictSpellToCastInMain2(ApiType exceptSA, boolean handOnly) SpellAbility
        +reserveManaSourcesForNextSpell(SpellAbility sa, SpellAbility exceptForSa) boolean
        +reserveManaSources(SpellAbility sa) boolean
        +reserveManaSources(SpellAbility sa, PhaseType phaseType, boolean enemy) boolean
        +reserveManaSources(SpellAbility sa, PhaseType phaseType, boolean enemy, boolean forNextSpell, SpellAbility exceptForThisSa) boolean
        -canPlayAndPayFor(SpellAbility sa) AiPlayDecision
        -canPlayAndPayForFace(SpellAbility sa) AiPlayDecision
        +canPlaySa(SpellAbility sa) AiPlayDecision
        -canPlaySpellOrLandBasic(Card card, SpellAbility sa) AiPlayDecision
        -canPlaySpellWithoutBuyback(Card card, SpellAbility sa) boolean
        +getCardsToDiscard(int numDiscard, String[] uTypes, SpellAbility sa) CardCollection
        +getCardsToDiscard(int numDiscard, String[] uTypes, SpellAbility sa, CardCollectionView exclude) CardCollection
        +getCardsToDiscard(int min, int max, CardCollection validCards, SpellAbility sa) CardCollection
        +confirmAction(SpellAbility sa, PlayerActionConfirmMode mode, String message, Map~String,Object~ params) boolean
        +confirmBidAction(SpellAbility sa, PlayerActionConfirmMode mode, String message, int bid, Player winner) boolean
        +confirmStaticApplication(Card hostCard, String logic) boolean
        +getProperty(AiProps propName) String
        +getIntProperty(AiProps propName) int
        +getBoolProperty(AiProps propName) boolean
        +canPlayFromEffectAI(Spell spell, boolean mandatory, boolean withoutPayingManaCost) AiPlayDecision
        +declareBlockersFor(Player defender, Combat combat) void
        +declareAttackers(Player attacker, Combat combat) void
        -removeUnpayableAttackers(Combat combat) void
        -singleSpellAbilityList(SpellAbility sa) List~SpellAbility~
        +chooseSpellAbilityToPlay() List~SpellAbility~
        -isSafeToHoldLandDropForMain2(Card landToPlay) boolean
        -getSpellAbilityToPlay() SpellAbility
        -chooseSpellAbilityToPlayFromList(List~SpellAbility~ all, boolean skipCounter) SpellAbility
        +chooseCardsToDelve(int genericCost, CardCollection grave) CardCollection
        +doTrigger(SpellAbility sa, boolean mandatory) boolean
        +aiShouldRun(CardTraitBase effect, SpellAbility sa, Card host, GameEntity affected) boolean
        ...
        +confirmPayment(CostPart costPart) boolean
        +attemptToAssist(SpellAbility sa, int max, int request) int
        +chooseCardsForEffect(CardCollectionView pool, SpellAbility sa, int min, int max, boolean isOptional, Map~String,Object~ params) CardCollection
        +complainCardsCantPlayWell(Deck myDeck) Map~DeckSection,List~
        +cheatShuffle(CardCollectionView in) CardCollectionView
        +chooseDirection(SpellAbility sa) boolean
        +chooseEvenOdd(SpellAbility sa) boolean
        ...
        +orderPlaySa(List~SpellAbility~ activePlayerSAs) List~SpellAbility~
        -filterList(List~T~ input, Predicate~T~ pred) List~T~
        +filterList(List~T~ input, Function~SpellAbility,Object~ pred, Object value) List~T~
        +filterListByApi(List~SpellAbility~ input, ApiType type) List~SpellAbility~
        -filterListByAiLogic(List~T~ list, String logic) List~T~
        ...
        -checkAiSpecificRestrictions(SpellAbility sa) boolean
        +chooseSingleReplacementEffect(List~ReplacementEffect~ list) ReplacementEffect
        +AiController(Player computerPlayer, Game game0)
    }
    AiController ..> AbilityKey : uses
    AiController ..> AbilityManaPart : uses
    AiController ..> AbilitySub : uses
    AiController ..> AiAttackController : uses
    AiController ..> AiBlockController : uses
    AiController ..> AiCardMemory : uses
    AiController ..> AiPlayDecision : uses
    AiController ..> AiProps : uses
    AiController ..> ApiType : uses
    AiController ..> Card : uses
    AiController ..> CardCollection : uses
    AiController ..> CardCollectionView : uses
    AiController ..> CardState : uses
    AiController ..> CardTraitBase : uses
    AiController ..> Color : uses
    AiController ..> Combat : uses
    AiController ..> Cost : uses
    AiController ..> CostDiscard : uses
    AiController ..> CostPart : uses
    AiController ..> CostPayLife : uses
    AiController ..> CostSacrifice : uses
    AiController ..> Deck : uses
    AiController ..> DeckSection : uses
    AiController ..> EmptySa : uses
    AiController ..> Game : uses
    AiController ..> GameEntity : uses
    AiController ..> GameEntityCounterTable : uses
    AiController ..> MagicColor : uses
    AiController ..> ManaCost : uses
    AiController ..> ManaCostBeingPaid : uses
    AiController ..> MemorySet : uses
    AiController ..> OptionalCostValue : uses
    AiController ..> PaperCard : uses
    AiController ..> PhaseType : uses
    AiController ..> Player : uses
    AiController ..> PlayerActionConfirmMode : uses
    AiController ..> PlayerCollection : uses
    AiController ..> ReplaceMoved : uses
    AiController ..> ReplacementEffect : uses
    AiController ..> ReplacementType : uses
    AiController ..> Spell : uses
    AiController ..> SpellAbility : uses
    AiController ..> SpellAbilityCondition : uses
    AiController ..> SpellAbilityPicker : uses
    AiController ..> SpellApiBased : uses
    AiController ..> SpellPermanent : uses
    AiController ..> StaticAbility : uses
    AiController ..> Trigger : uses
    AiController ..> TriggerReplacementBase : uses
    AiController ..> WrappedAbility : uses
    AiController ..> ZoneType : uses
```
...
- [[forge.ai.AiCardMemory|AiCardMemory]]
...
AiController is the AI player's central decision-making controller in Forge's `forge-ai` module, owning the `Player` it drives and the `Game` it plays in. It answers the full range of choices the rules engine poses of a controller: which spell or ability to play, when and which land to drop, what to discard, how to declare attackers and blockers, and how to resolve triggers, replacement effects, modal choices, and numeric or confirmation prompts. It returns `AiPlayDecision` verdicts through `canPlaySa`/`canPlayAndPayFor`, delegating combat to `AiAttackController` and `AiBlockController`, per-API tactics to `SpellApiToAi`, and optionally a simulation-based `SpellAbilityPicker`.

The design favors tunable heuristics with safety guards: it caches predicted combat, reserves mana via `AiCardMemory`, reads behavior from `AiProps` profiles, and evaluates each candidate play inside a timeout-bounded worker thread so infinite loops can't hang the engine. Profile properties and special-card hooks let it model deck archetypes (Momir, Living End, Storm) without hardcoding them into the core loop.
