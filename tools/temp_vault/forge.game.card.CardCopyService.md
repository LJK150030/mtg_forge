...
tags:
  ...
  - module/forge-game
  - pkg/forge/game/card
fqn: forge.game.card.CardCopyService
...
classDiagram
    class CardCopyService {
        -Card copyFrom
        -Game toGame
        +copyCard(boolean assignNewId) Card
        +copyCard(boolean assignNewId, Player owner) Card
        -copyState(Card from, CardStateName fromState, Card to, CardStateName toState) void
        -copyState(Card from, CardStateName fromState, Card to, CardStateName toState, boolean updateView) void
        +copyStats(Card in, Player newOwner, boolean assignNewId) Card
        +copyCopiableCharacteristics(Card to, SpellAbility sourceSA, SpellAbility targetSA) void
        +getLKICopyList(Iterable~Card~ in, Map~Integer,Card~ cachedMap) List~Card~
        +getLKICopy(Card c) Card
        +getLKICopy(Card c, Map~Integer,Card~ cachedMap) Card
        +getLKICopy(GameEntity c, Map~Integer,Card~ cachedMap) GameEntity
        +getLKICopy() Card
        +getLKICopy(Map~Integer,Card~ cachedMap) Card
        +CardCopyService(Card copyFrom)
        +CardCopyService(Card copyFrom, Game toGame)
    }
    CardCopyService ..> Card : uses
    CardCopyService ..> CardState : uses
    CardCopyService ..> CardStateName : uses
    CardCopyService ..> CardType : uses
    CardCopyService ..> DetachedCardEffect : uses
    CardCopyService ..> Game : uses
    CardCopyService ..> GameEntity : uses
    CardCopyService ..> Player : uses
    CardCopyService ..> SpellAbility : uses
```
...
CardCopyService is a stateless-style service that centralizes all logic for duplicating a `Card`, consolidating copy routines that previously lived in `CardFactory` and `CardUtil`. Constructed around a source card (`copyFrom`) and an optional destination `Game`, it offers two distinct copy strategies: faithful runtime copies via `copyCard`/`copyStats` (used when moving cards between zones or cloning entire games), and last-known-information (LKI) snapshots via the `getLKICopy` family, which freeze a card's characteristics, P/T, counters, and status flags for rules that reference a card as it last existed.

It collaborates heavily with `Card` and its per-`CardStateName` `CardState` characteristics, handling multi-faced layouts (flip, transform, split, meld) explicitly, and special-cases `DetachedCardEffect`. Static convenience overloads and a shared `Map<Integer,Card>` cache prevent redundant copies and infinite recursion across linked entities (`GameEntity`, attachments), while Sentry breadcrumbs aid diagnostics. The deprecated `copyCopiableCharacteristics` encodes specific MTG copy rules (e.g., 707.10g).
