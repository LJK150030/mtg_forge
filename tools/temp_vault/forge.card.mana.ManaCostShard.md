...
tags:
  ...
  - module/forge-core
  - pkg/forge/card/mana
fqn: forge.card.mana.ManaCostShard
...
**Package:** `forge.card.mana` &nbsp; **Module:** `forge-core` &nbsp; **Kind:** Enum
...
classDiagram
    class ManaCostShard {
        ...
        -String imageKey
        +int COLORS_SUPERPOSITION
        -getCMC() int
        -getCmpCost() float
        +getColorMask() byte
        +getColor() ColorSet
        +valueOf(int atoms) ManaCostShard
        +parseNonGeneric(String unparsed) ManaCostShard
        +toString() String
        ...
        +isColorless() boolean
        +isGeneric() boolean
        +isOr2Generic() boolean
        +isColor(byte colorCode) boolean
        +canBePaidWithManaOfColor(byte colorCode) boolean
        +isOfKind(int atom) boolean
        +getShard() int
        ~ManaCostShard(int value, String sValue)
    }
    ManaCostShard ..> ColorSet : uses
```
...
## Source
`forge-core/src/main/java/forge/card/mana/ManaCostShard.java`
