...
classDiagram
    class FogAi {
        ...
        -handleMemoryCheck(Player ai, SpellAbility sa) boolean
        -countAvailableFogs(Player ai) int
    ...
    FogAi --|> SpellAbilityAi : extends
    FogAi ..> AiAbilityDecision : uses
    FogAi ..> Card : uses
    FogAi ..> Combat : uses
    FogAi ..> Game : uses
    FogAi ..> GameObject : uses
    FogAi ..> Player : uses
    FogAi ..> PlayerControllerAi : uses
    FogAi ..> SpellAbility : uses
```
...
FogAi is a concrete AI strategy class in Forge's `forge.ai.ability` package that decides when the computer player should cast or activate "Fog"-style effects, which prevent combat damage for a turn. Extending `SpellAbilityAi`, it overrides the framework hooks `canPlay`, `chkDrawback`, and `doTriggerNoCost` to plug Fog-specific reasoning into the engine's generic spell-evaluation pipeline. It collaborates with the `Game`, `Combat`, `PhaseHandler`, and `Player` model to gauge incoming damage and timing, returning an `AiAbilityDecision` that scores the play.

Its design intent centers on timing and resource conservation: it holds Fogs until the opponent's declare-blockers step, only casts when life is genuinely threatened or a Fog-heavy deck can afford tempo plays, and uses `AiCardMemory` plus `PlayerControllerAi`'s mana reservation to pre-commit mana for a reactive, last-moment defensive cast.
