...
fqn: forge.game.spellability.AbilityManaPart
...
classDiagram
    class AbilityManaPart {
        ...
        -String origProduced
        -String lastExpressChoice
        -String manaRestrictions
        -String extraManaRestrictions
        -String cannotCounterSpell
        -String addsKeywords
        -String addsKeywordsType
        -String addsKeywordsUntil
        -String addsCounters
        -String triggersWhenSpent
        -boolean persistentMana
        -boolean combatMana
        -List~Mana~ lastManaProduced
        -Card sourceCard
        -IHasSVars sVarHolder
        +produceMana(SpellAbility sa) void
        +produceMana(String produced, Player player, SpellAbility sa) String
        +tapsForMana(SpellAbility root, String mana) void
        +cannotCounterPaidWith(SpellAbility saBeingPaid) boolean
        +isCannotCounterPaidWith() boolean
        +addNoCounterEffect(SpellAbility saBeingPaid) void
        +addKeywords(SpellAbility saBeingPaid) boolean
        +getAddsKeywordsType() String
        +getAddsKeywordsUntil() String
        +getKeywords() String
        +addsCounters(SpellAbility saBeingPaid) boolean
        +createETBCounters(Card c, Player controller) void
        +getTriggersWhenSpent() boolean
        +addTriggersWhenSpent(SpellAbility saBeingPaid) void
        +getSourceSA() SpellAbility
        +getManaRestrictions() String
        +setExtraManaRestriction(String str) void
        +getExtraManaRestriction() String
        +meetsManaRestrictions(SpellAbility sa) boolean
        +meetsManaRestrictions(SpellAbility sa, String restrictions) boolean
        +meetsManaShardRestrictions(ManaCostShard shard, byte color) boolean
        +meetsSpellAndShardRestrictions(SpellAbility sa, ManaCostShard shard, byte color) boolean
        +mana(SpellAbility sa) String
        +setExpressChoice(String s) void
        +setExpressChoice(ColorSet cs) void
        +getExpressChoice() String
        +clearExpressChoice() void
        +getLastManaProduced() List~Mana~
        +isSnow() boolean
        +isAnyMana() boolean
        +isComboMana() boolean
        +isSpecialMana() boolean
        +canProduce(String s, SpellAbility sa) boolean
        +equals(Object o) boolean
        +hashCode() int
        +getOrigProduced() String
        +getComboColors(SpellAbility sa) String
        +getChosenColor(SpellAbility sa) String
        +getSourceCard() Card
        +setSourceCard(Card host) void
        +isPersistentMana() boolean
        +isCombatMana() boolean
        ~abilityProducesManaColor(SpellAbility am, byte neededColor) boolean
        +AbilityManaPart(SpellAbility sourceSA, Map~String,String~ params)
        +AbilityManaPart(Card sourceCard, Map~String,String~ params)
        +AbilityManaPart(Card newSource, AbilityManaPart oldMana)
    }
    AbilityManaPart ..|> Serializable : implements
    AbilityManaPart ..> AbilityKey : uses
    AbilityManaPart ..> Card : uses
    AbilityManaPart ..> ColorSet : uses
    AbilityManaPart ..> Cost : uses
    AbilityManaPart ..> Game : uses
    AbilityManaPart ..> IHasSVars : uses
    AbilityManaPart ..> Mana : uses
    AbilityManaPart ..> ManaCostShard : uses
    AbilityManaPart ..> ManaPool : uses
    AbilityManaPart ..> Player : uses
    AbilityManaPart ..> ReplacementEffect : uses
    AbilityManaPart ..> SpellAbility : uses
    AbilityManaPart ..> Trigger : uses
    AbilityManaPart ..> Zone : uses
    AbilityManaPart ..> ZoneType : uses
```
