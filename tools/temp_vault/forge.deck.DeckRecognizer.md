...
classDiagram
    class DeckRecognizer {
        -Pattern SEARCH_SINGLE_SLASH
        -String DOUBLE_SLASH
        -String LINE_COMMENT_DELIMITER_OR_MD_HEADER
        -String ASTERISK
        +String REGRP_DECKNAME
        +String REX_DECK_NAME
        ...
        +String REX_RARITY
        +String MANA_SYMBOLS
        +String REX_MANA_SYMBOLS
        +String REX_MANA_COLOURS
        +String REX_MANA
        +Pattern NONCARD_PATTERN
        +Pattern CMC_PATTERN
        +Pattern CARD_RARITY_PATTERN
        +Pattern MANA_PATTERN
        +Pattern MANA_SYMBOL_PATTERN
        +String REGRP_SET
        +String REGRP_COLLNR
        +String REGRP_CARD
        +String REGRP_CARDNO
        +String REX_CARD_NAME
        +String REX_SET_CODE
        +String REX_COLL_NUMBER
        +String REX_CARD_COUNT
        +String REGRP_FOIL_GFISH
        -String REX_FOIL_MTGGOLDFISH
        +String REGRP_DECK_SEC_XMAGE_STYLE
        -String REX_DECKSEC_XMAGE
        +String REX_CARD_SET_REQUEST
        +Pattern CARD_SET_PATTERN
        +String REX_SET_CARD_REQUEST
        +Pattern SET_CARD_PATTERN
        +String REX_FULL_REQUEST_CARD_SET
        +Pattern CARD_SET_COLLNO_PATTERN
        +String REX_FULL_REQUEST_SET_CARD
        +Pattern SET_CARD_COLLNO_PATTERN
        +String REX_FULL_REQUEST_CARD_COLLNO_SET
        +Pattern CARD_COLLNO_SET_PATTERN
        +String REX_FULL_REQUEST_XMAGE
        +Pattern SET_COLLNO_CARD_XMAGE_PATTERN
        +String REX_CARDONLY
        +Pattern CARD_ONLY_PATTERN
        -CharSequence[] CARD_TYPES
        -CharSequence[] DECK_SECTION_NAMES
        -Date releaseDateConstraint
        -List~String~ allowedSetCodes
        -List~String~ gameFormatBannedCards
        -List~String~ gameFormatRestrictedCards
        -List~DeckSection~ allowedDeckSections
        -boolean includeBannedAndRestricted
        -DeckFormat deckFormat
        -CardDb.CardArtPreference artPreference
        -allCardTypes() CharSequence[]
        +parseCardList(String[] cardList) List~Token~
        -isAllowed(DeckSection tokenSection) boolean
        +recognizeLine(String rawLine, DeckSection referenceSection) Token
        +purgeAllLinks(String line) String
        +recogniseCardToken(String text, DeckSection currentDeckSection) Token
        -checkDoubleSidedCard(String cardName) String
        -checkAndSetCardToken(PaperCard pc, CardEdition edition, int cardCount, String deckSecFromCardLine, DeckSection referenceSection, boolean cardRequestHasSetCode) Token
        -getTokenSection(String deckSec, DeckSection currentDeckSection, PaperCard card) DeckSection
        -hasGameFormatConstraints() boolean
        -getRexGroup(Matcher matcher, String groupName) String
        -isBannedInFormat(PaperCard pc) boolean
        -isRestrictedInFormat(PaperCard pc, int cardCount) boolean
        -IsIllegalInFormat(String setCode) boolean
        -isNotCompliantWithReleaseDateRestrictions(CardEdition edition) boolean
        -getRegExMatchers(String line) List~Matcher~
        +recogniseNonCardToken(String text) Token
        +isCardType(String lineAsIs) boolean
        +isCardRarity(String lineAsIs) boolean
        +isCardCMC(String lineAsIs) boolean
        +isManaToken(String lineAsIs) boolean
        +isDeckSectionName(String lineAsIs) boolean
        -nonCardTokenMatch(String lineAsIs) String
        -cardRarityTokenMatch(String lineAsIs) String
        -cardCMCTokenMatch(String lineAsIs) String
        -getCardCMCMatch(String lineAsIs) String
        -manaTokenMatch(String lineAsIs) Pair~String,String~
        -matchAnyManaSymbolIn(String manaToken) String
        -getManaTokenMatch(String lineAsIs) String
        -getMagicColourLabel(MagicColor.Color magicColor) String
        -getMagicColourLabel(MagicColor.Color magicColor1, MagicColor.Color magicColor2) String
        -getMagicColor(String colorName) MagicColor.Color
        +isDeckName(String lineAsIs) boolean
        +deckNameMatch(String text) String
        +setDateConstraint(int year, int month) void
        +setGameFormatConstraint(List~String~ allowedSetCodes, List~String~ bannedCards, List~String~ restrictedCards) void
        +setDeckFormatConstraint(DeckFormat deckFormat0) void
        +setArtPreference(CardDb.CardArtPreference artPref) void
        +setAllowedDeckSections(List~DeckSection~ deckSections) void
        +forceImportBannedAndRestrictedCards() void
    }
    DeckRecognizer ..> CardArtPreference : uses
    DeckRecognizer ..> CardDb : uses
    DeckRecognizer ..> CardEdition : uses
    DeckRecognizer ..> CardType : uses
    DeckRecognizer ..> Color : uses
    DeckRecognizer ..> CoreType : uses
    DeckRecognizer ..> DeckFormat : uses
    DeckRecognizer ..> DeckSection : uses
    DeckRecognizer ..> LimitedCardType : uses
    DeckRecognizer ..> MagicColor : uses
    DeckRecognizer ..> PaperCard : uses
    DeckRecognizer ..> StaticData : uses
    DeckRecognizer ..> Token : uses
    DeckRecognizer ..> TokenKey : uses
    DeckRecognizer ..> TokenType : uses
```
