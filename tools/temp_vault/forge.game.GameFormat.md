...
classDiagram
    class GameFormat {
        ...
        #List~String~ bannedCardNames
        #List~String~ restrictedCardNames
        #List~String~ additionalCardNames
        #boolean restrictedLegendary
        -Date effectiveDate
        -SimpleDateFormat formatter
        -String DEFAULTDATE
        #List~String~ allowedSetCodes_ro
        #List~String~ bannedCardNames_ro
        #List~String~ restrictedCardNames_ro
        #List~String~ additionalCardNames_ro
        #Predicate~PaperCard~ filterRules
        #Predicate~PaperCard~ filterPrinted
        -int index
        +GameFormat NoFormat
        +Predicate~CardEdition~ editionLegalPredicate
        #buildFilter(boolean printed) Predicate~PaperCard~
        -buildFilterPrinted() Predicate~PaperCard~
        -buildFilterRules() Predicate~PaperCard~
        +getName() String
        -parseDate(String date) Date
        +getEffectiveDate() Date
        +getFormatType() FormatType
        +getFormatSubType() FormatSubType
        +getAllowedSetCodes() List~String~
        +getBannedCardNames() List~String~
        +getRestrictedCards() List~String~
        +isRestrictedLegendary() Boolean
        +getAdditionalCards() List~String~
        +getAllowedRarities() List~CardRarity~
        +getAllCards() List~PaperCard~
        +getFilterRules() Predicate~PaperCard~
        +getFilterPrinted() Predicate~PaperCard~
        +isSetLegal(String setCode) boolean
        -isPoolLegal(CardPool allCards) boolean
        +isDeckLegal(Deck deck) boolean
        -getPoolLegalityProblem(CardPool allCards) String
        +getDeckConformanceProblem(Deck deck) String
        +toString() String
        +compareTo(GameFormat other) int
        +getIndex() int
        +GameFormat(String fName, Iterable~String~ sets, List~String~ bannedCards)
        +GameFormat(String fName, Date effectiveDate, Iterable~String~ sets, List~String~ bannedCards, List~String~ restrictedCards, Boolean restrictedLegendary, List~String~ additionalCards, List~CardRarity~ rarities, int compareIdx, FormatType formatType, FormatSubType formatSubType)
    }
    GameFormat ..|> Comparable : implements
    GameFormat ..> CardDb : uses
    GameFormat ..> CardEdition : uses
    GameFormat ..> CardPool : uses
    GameFormat ..> CardRarity : uses
    GameFormat ..> Deck : uses
    GameFormat ..> EditionEntry : uses
    GameFormat ..> FileSection : uses
    GameFormat ..> FormatSubType : uses
    GameFormat ..> FormatType : uses
    GameFormat ..> InverseDateComparator : uses
    GameFormat ..> PaperCard : uses
    GameFormat ..> Reader : uses
    GameFormat ..> StaticData : uses
    GameFormat ..> StorageBase : uses
    GameFormat ..> StorageReaderRecursiveFolderWithUserFolder : uses
```
